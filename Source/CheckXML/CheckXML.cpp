...
double GetXMLReducedSpeed(const xml_node& node, const std::string& name) {
    xml_node unit_node = node.child("Units");
    std::string unit = unit_node.attribute("ReducedSpeed").value();
    return GetXMLReducedSpeed(node, name, unit);
}

double GetXMLReducedSpeed(const xml_node& node, const std::string& name, const std::string& unit) {
    double x = GetAttributeAsDouble(node,name.c_str());
    return to_rps_sqrt_K(x,unit);
}

double GetXMLReducedMassFlow(const xml_node& node, const std::string& name){
    xml_node unit_node = node.child("Units");
    std::string unit = unit_node.attribute("ReducedMassFlow").value();
    return GetXMLReducedMassFlow(node, name, unit);
}

double GetXMLReducedMassFlow(const xml_node& node, const std::string& name, const std::string& unit) {
    double x = GetAttributeAsDouble(node,name.c_str());
    return to_kg_s_sqrt_K_MPa(x,unit);
}
...
double GetXMLVolume(const xml_node& node,const std::string& name) {
    xml_node unit_node = node.child("Units");
    std::string unit = unit_node.attribute("Volume").value();
    return GetXMLVolume(node, name, unit);
}

double GetXMLVolume(const xml_node& node,const std::string& name,const std::string& unit) {
    double x = GetAttributeAsDouble(node,name.c_str());
    return to_cubic_metres(x, unit);
}
