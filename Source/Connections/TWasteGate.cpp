...
fscanf(fich,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d",&FTuboControl,&FDistancia,
     &FPresCapMando,&FCoefC1,&FCoefC2,&FMasa,&FAmortiguamiento,&FRigidez,&FPrecarga,
     &FAreaDiaf,&FAreaPlato,&FDiametroRef,&modo);

switch(modo){
     case 0: FSentidoWG=1; break;
     case 1: FSentidoWG=-1; break;
}
...
if(FNodoAdm>=Pipe[FTuboControl-1]->getNin()-1){
  FNodoAdm = FNodoAdm-1;
  FDist=1;
}else FDist=FDistancia/Pipe[FTuboControl-1]->getXRef()-FNodoAdm;
