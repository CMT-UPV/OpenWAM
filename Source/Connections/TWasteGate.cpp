...
void TWasteGate::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor,
	...
	try {
		...
		FCoefC1 = GetAttributeAsDouble(node_wg, "Coef1");
		FCoefC2 = GetAttributeAsDouble(node_wg, "Coef2");
		FMasa = GetXMLMass(node_wg, "Mass");
		FAmortiguamiento = GetXMLDamping(node_wg, "Damping");
		FRigidez = GetXMLStiffness(node_wg, "Stiffness");
		FPrecarga = GetXMLForce(node_wg, "Preload");
		FAreaDiaf = GetXMLArea(node_wg, "DiafArea");
		FAreaPlato = GetXMLArea(node_wg, "PlateArea");
