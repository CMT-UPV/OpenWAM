...
void TValvula4T::LeeDatosInicialesXML(xml_node node_valve, int norden,
	...
	try {
		...
		const char_t* SpeedControl =
				node_4svalve.attribute("SpeedControl").value();
		if (SpeedControl == "Independent") {
			FRelacionVelocidades = 1.;
			FRegimen = GetAttributeAsDouble(node_4svalve, "Speed");
		} else if (SpeedControl == "ByEngine") {
			FRelacionVelocidades = GetAttributeAsDouble(node_4svalve,
					"SpeedRatio");
		}

		const char_t* Parameter;

		for (xml_node node_ctrl = GetNodeChild(node_4svalve, "Actuator");
