...

{
	try {
		...
		if (EngineBlock) {
			...
			Engine[0] = new TBloqueMotor(AmbientPressure, AmbientTemperature, SpeciesModel, SpeciesNumber,
...
void TOpenWAM::ReadPipes() {
	try {
		...
		for (int i = 0; i < NumberOfPipes; i++) {
			Pipe[i] = new TTubo(SpeciesNumber, i, SimulationDuration, Engine, SpeciesModel, GammaCalculation,
				ThereIsEGR);
			Pipe[i]->LeeDatosGeneralesTubo(fileinput.c_str(), filepos);
			if (EngineBlock) {
				Pipe[i]->LeeDatosGeometricosTubo(fileinput.c_str(), filepos, Engine[0]->getRegimen(), tipomallado,
					Engine);
			} else {
		...
		if (NumberOfPlenums != 0) {
			for (int i = 0; i < NumberOfPlenums; ++i) {
				...
				case 1:
					...
					Plenum[i]->LeeDatosGeneralesDepositos(fileinput.c_str(), filepos);
					dynamic_cast<TDepVolVariable*>(Plenum[i])->LeeDatosDepVolVariable(fileinput.c_str(), filepos,
						EngineBlock);
					ncv++;
					break;
				case 2:
				...
				case 5:
					...
					Plenum[i] = new TUnionDireccional(i, NumberOfDirectionalJunctions, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					Plenum[i]->LeeDatosGeneralesDepositos(fileinput.c_str(), filepos);
...
void TOpenWAM::ReadConnections() {
	try {
		...
		NumTCCUnionEntreTubos = 0, NumTCCCilindro = 0, NumTCCDeposito = 0, NumTCCRamificacion = 0,
		...
		fscanf(FileInput, "%d %d %d %d %d %d %d %d %d", &numnodosimples, &numpulsos, &numnododep, &numperdpresion,
			&numcomprtornillo, &numextremosinyeccion, &numnodoentredepositos, &numentradacompresor,
		...
		if (NumberOfConnections != 0) {
			for (int i = 0; i <= NumberOfConnections - 1; ++i) {
				...
				switch (TipoCC) {
				case 0:
					BC[i] = new TCCDescargaExtremoAbierto(nmOpenEndAtmosphere, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					...
					break;
				case 1:
					BC[i] = new TCCDescargaExtremoAbierto(nmOpenEndReservoir, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					NumTCCDescargaExtremoAbierto++;
#ifdef ParticulateFilter
					BC[i]->ReadBoundaryData(fileinput.c_str(), filepos, NumberOfPipes,
					...
					break;
				case 2:
					BC[i] = new TCCDescargaExtremoAbierto(nmOpenEndCalcExtern, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					NumTCCDescargaExtremoAbierto++;
					nematlab++;
#ifdef ParticulateFilter
					...
					break;
				case 3:
					BC[i] = new TCCExtremoCerrado(nmClosedEnd, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					NumTCCExtremoCerrado++;
					BC[i]->AsignaTubos(NumberOfPipes, Pipe);
					break;
				case 4:
					BC[i] = new TCCExtremoAnecoico(nmAnechoicEnd, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					NumTCCExtremoAnecoico++;
					BC[i]->AsignaTubos(NumberOfPipes, Pipe);
					break;
				case 5:
					BC[i] = new TCCPulso(nmIncidentPressurWave, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					NumTCCPulso++;
#ifdef ParticulateFilter
					BC[i]->ReadBoundaryData(fileinput.c_str(), filepos, NumberOfPipes,
					...
					break;
				case 6:
					BC[i] = new TCCUnionEntreTubos(nmPipesConnection, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					...
					break;
				case 7:
					BC[i] = new TCCCilindro(nmIntakeValve, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					NumTCCCilindro++;
					...
					break;
				case 8:
					BC[i] = new TCCCilindro(nmExhaustValve, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					NumTCCCilindro++;
					...
					break;
				case 9:
					BC[i] = new TCCPerdidadePresion(nmLinearPressureLoss, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					...
					break;
				case 10:
					BC[i] = new TCCPerdidadePresion(nmQuadraticPressureLoss, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					NumTCCPerdidaPresion++;
#ifdef ParticulateFilter
					BC[i]->ReadBoundaryData(fileinput.c_str(), filepos, NumberOfPipes,
					...
					break;
				case 11:
					BC[i] = new TCCDeposito(nmPipeToPlenumConnection, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					NumTCCDeposito++;
#ifdef ParticulateFilter
					BC[i]->ReadBoundaryData(fileinput.c_str(), filepos, NumberOfPipes,
				...
				case 13:
					...
					fgetpos(FileInput, &filepos);
					fclose(FileInput);
					BC[i] = new TCCCompresorVolumetrico(nmVolumetricCompressor, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					dynamic_cast<TCCCompresorVolumetrico*>(BC[i])->PutNumeroCV(numerocv);
					NumberOfVolumetricCompressors++;
					dynamic_cast<TCCCompresorVolumetrico*>(BC[i])->LeeCCCompresorVol(fileinput.c_str(), filepos,
						NumberOfPipes, Pipe, EngineBlock);
					dynamic_cast<TCCCompresorVolumetrico*>(BC[i])->IniciaMedias();
					break;
				case 14:
					BC[i] = new TCCExtremoInyeccion(nmInjectionEnd, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					...
					break;
				case 15:
					BC[i] = new TCCEntradaCompresor(nmEntradaCompre, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
					...
					break;
				case 16:
					BC[i] = new TCCUnionEntreDepositos(nmUnionEntreDepositos, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
					NumberOfConectionsBetweenPlenums++;
					dynamic_cast<TCCUnionEntreDepositos*>(BC[i])->LeeUEDepositos(fileinput.c_str(), filepos,
						Independent);
					break;
				case 17:
				...
				case 18:
					...
					break;
				case 19:
					BC[i] = new TCFDConnection(nmCFDConnection, i, SpeciesModel, SpeciesNumber, GammaCalculation,
						ThereIsEGR);
#ifdef ParticulateFilter
					BC[i]->ReadBoundaryData(fileinput.c_str(), filepos, NumberOfPipes,
					Pipe, NumberOfDPF, DPF);
					...
					break;
				case 20:
					BC[i] = new TCCExternalConnectionVol(nmExternalConnection, i, SpeciesModel, SpeciesNumber,
						GammaCalculation, ThereIsEGR);
				...
				if (BC[i]->getTipoCC() == nmIntakeValve || BC[i]->getTipoCC() == nmExhaustValve
					...
					if (BC[i]->getTipoCC() == nmIntakeValve || BC[i]->getTipoCC() == nmExhaustValve) {
						...
						dynamic_cast<TCCUnionEntreDepositos*>(BC[i])->AsignaTipoValvula(TypeOfValve, quevalv,
							numerovalvula);
						if (TypeOfValve[quevalv - 1]->getTypeOfValve() == nmLamina)
							NumberOfReedValves++;
						if (TypeOfValve[quevalv - 1]->getTypeOfValve() == nmWasteGate)
...
#else
	Output->ReadAverageResults(fileinput.c_str(), filepos, Pipe, EngineBlock, Engine, Plenum, Axis, Compressor, Turbine,
		BC, NULL, VolumetricCompressor, Venturi, Sensor, Controller, SimulationDuration, FileName);

	Output->ReadInstantaneousResults(fileinput.c_str(), filepos, Engine, Plenum, Pipe, Venturi, BC, NULL, Axis,
		Compressor, Turbine, VolumetricCompressor, BCWasteGate, NumberOfWasteGates, BCReedValve, NumberOfReedValves,
		Sensor, Controller, FileName);
#endif
			...
			for (int j = 0; j < NumberOfConnections; j++) {
				if (BC[j]->getTipoCC() == nmPipeToPlenumConnection) {
					if (dynamic_cast<TCCDeposito*>(BC[j])->getValvula()->getTypeOfValve() == nmStator) {
						if (dynamic_cast<TEstatorTurbina*>(dynamic_cast<TCCDeposito*>(BC[j])->getValvula())
							->getNumeroTurbina() == i + 1) {
							entr = dynamic_cast<TEstatorTurbina*>(dynamic_cast<TCCDeposito*>(BC[j])->getValvula())
								->getNumeroEntrada() - 1;
							StatorTurbine[i][entr] = dynamic_cast<TEstatorTurbina*>(dynamic_cast<TCCDeposito*>(BC[j])
								->getValvula());
						}
					} else if (dynamic_cast<TCCDeposito*>(BC[j])->getValvula()->getTypeOfValve() == nmRotor) {
						if (dynamic_cast<TRotorTurbina*>(dynamic_cast<TCCDeposito*>(BC[j])->getValvula())
							->getNumeroTurbina() == i + 1) {
							RotorTurbine[i] =
				...
				if (RotorTurbine[i]->getTipoRotor() == nmRotVariable) {
					...
					for (int j = 0; j < Turbine[i]->getNumeroEntradas(); ++j) {
						if (Turbine[i]->GetCCEntrada(j)->getTipoCC() == nmPipeToPlenumConnection) {
							DatosTGV[tgv].Estator[j] = dynamic_cast<TCCDeposito*>(Turbine[i]->GetCCEntrada(j))
								->getValvula();
						} else if (Turbine[i]->GetCCEntrada(j)->getTipoCC() == nmUnionEntreDepositos) {
							DatosTGV[tgv].Estator[j] =
								dynamic_cast<TCCUnionEntreDepositos*>(Turbine[i]->GetCCEntrada(j))->getValvula();
						}
	...
	try {
		...
		for (int i = 0; i < NumberOfExhaustValves; i++) {
			...
			for (int j = 0; j < NumberOfPipes; j++) {
				...
				if (Encontrado) {
					CalculateDistance(NodoOrigen, NodoFin, Pipe[j]->getLongitudTotal(), NumberOfPlenums, NumberOfPipes,
			...
			for (int i = 0; i < NumberOfPlenums; i++) {
						...
						for (int k = 0; k < NumberOfConnections; k++) {
							if (BC[k]->getTipoCC() == nmPipeToPlenumConnection) {
								if (dynamic_cast<TCCDeposito*>(BC[NodoOrigen - 1])->getNumeroDeposito()
									...
									NodoFin = k + 1;
									CalculateDistance(NodoOrigen, NodoFin, 0., NumberOfPlenums, NumberOfPipes,
										NumberOfConnections, Pipe, BC);
								}
							}
							if (BC[k]->getTipoCC() == nmUnionEntreDepositos) {
								if (dynamic_cast<TCCDeposito*>(BC[NodoOrigen - 1])->getNumeroDeposito()
									...
									|| dynamic_cast<TCCDeposito*>(BC[NodoOrigen - 1])->getNumeroDeposito()
				...
				if (BC[NodoOrigen - 1]->getTipoCC() == nmUnionEntreDepositos) {
					...
					} else if (dynamic_cast<TCCUnionEntreDepositos*>(BC[NodoOrigen - 1])->getNumeroDeposito2()
						== i + 1) {
						for (int k = 0; k < NumberOfConnections; k++) {
							if (BC[k]->getTipoCC() == nmPipeToPlenumConnection) {
		...
		if (LBC[LNodoFin - 1]->getPosicionNodo() > LBC[LNodoOrigen - 1]->getPosicionNodo() + Long) {
			...
			for (int i = 0; i < LNumberOfPipes; i++) {
				if (LPipe[i]->getNodoDer() == LNodoFin && LPipe[i]->getNodoIzq() != LNodoOrigen) {
					...
					CalculateDistance(NodoOrigen1, NodoFin1, LPipe[i]->getLongitudTotal(), LNumberOfPlenums,
			...
			for (int i = 0; i < LNumberOfPlenums; i++) {
						...
						for (int k = 0; k < LNumberOfConnections; k++) {
							if (LBC[k]->getTipoCC() == nmPipeToPlenumConnection) {
								if (dynamic_cast<TCCDeposito*>(LBC[LNodoOrigen - 1])->getNumeroDeposito()
									...
									NodoOrigen1 = LNodoFin;
									NodoFin1 = k + 1;
									CalculateDistance(NodoOrigen1, NodoFin1, 0, LNumberOfPlenums, LNumberOfPipes,
							...
							if (LBC[k]->getTipoCC() == nmUnionEntreDepositos) {
								if (dynamic_cast<TCCDeposito*>(LBC[LNodoOrigen - 1])->getNumeroDeposito()
									...
									|| dynamic_cast<TCCDeposito*>(LBC[LNodoOrigen - 1])->getNumeroDeposito()
				...
				if (LBC[LNodoOrigen - 1]->getTipoCC() == nmUnionEntreDepositos) {
					...
					} else if (dynamic_cast<TCCUnionEntreDepositos*>(LBC[LNodoOrigen - 1])->getNumeroDeposito2()
						== i + 1) {
						for (int k = 0; k < LNumberOfConnections; k++) {
							if (LBC[k]->getTipoCC() == nmPipeToPlenumConnection) {
			...
			if (EngineBlock) {
				do {
					...
					for (int j = 0; j < Engine[0]->GetCilindro(i)->getNumeroUnionesAdm(); ++j) {
						gasta +=
							dynamic_cast<TCCCilindro*>(Engine[0]->GetCilindro(i)->GetCCValvulaAdm(j))->getMassflow();
					}
					gaste = 0.;
					for (int j = 0; j < Engine[0]->GetCilindro(i)->getNumeroUnionesEsc(); ++j) {
						gaste +=
							dynamic_cast<TCCCilindro*>(Engine[0]->GetCilindro(i)->GetCCValvulaEsc(j))->getMassflow();
					}
			...
			if (LPlenum[LNumDepInicial - 1]->getNUniones() != 0) {
				for (int i = 0; i < LPlenum[LNumDepInicial - 1]->getNUniones(); i++) {
					if (LPlenum[LNumDepInicial - 1]->GetCCDeposito(i)->getUnionDPF()) {
						...
						if (LPlenum[LNumDepInicial - 1]->GetCCDeposito(i)->GetTuboExtremo(0).Pipe->getTime1()
							< *LTMinimo) {
							*LTMinimo =
								LPlenum[LNumDepInicial - 1]->GetCCDeposito(i)->GetTuboExtremo(0).Pipe->getTime1();
						}
					}
				}
			}
		...
		if (LPlenum[LNumDepInicial - 1]->getNUnionesED() != 0) {
			for (int j = 0; j < LPlenum[LNumDepInicial - 1]->getNUnionesED(); j++) {
				if (LPlenum[LNumDepInicial - 1]->GetCCUnionEntreDep(j)->getTipoCC() == nmUnionEntreDepositos) {
					...
					if (dynamic_cast<TCCUnionEntreDepositos*>(LPlenum[LNumDepInicial - 1]->GetCCUnionEntreDep(j))
						->getNumeroDeposito1() == LNumDepInicial) {
						NumDepSiguiente = dynamic_cast<TCCUnionEntreDepositos*>(LPlenum[LNumDepInicial - 1]
							->GetCCUnionEntreDep(j))->getNumeroDeposito2();
					} else
						NumDepSiguiente = dynamic_cast<TCCUnionEntreDepositos*>(LPlenum[LNumDepInicial - 1]
							->GetCCUnionEntreDep(j))->getNumeroDeposito1();

				}
				if (LPlenum[LNumDepInicial - 1]->GetCCUnionEntreDep(j)->getTipoCC() == nmCompresor) {
					if (dynamic_cast<TCCCompresor*>(LPlenum[LNumDepInicial - 1]->GetCCUnionEntreDep(j))
						->getNumeroDepositoRot() == LNumDepInicial) {
						NumDepSiguiente =
							dynamic_cast<TCCCompresor*>(LPlenum[LNumDepInicial - 1]->GetCCUnionEntreDep(j))
		...
		if (Plenum[LNumDeposito - 1]->getNUnionesED() != 0) {
			for (int j = 0; j < Plenum[LNumDeposito - 1]->getNUnionesED(); j++) {
				if (Plenum[LNumDeposito - 1]->GetCCUnionEntreDep(j)->getTipoCC() == nmUnionEntreDepositos) {
					...
					if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[LNumDeposito - 1]->GetCCUnionEntreDep(j))
						->getNumeroDeposito1() == LNumDeposito) {
						NumDepSiguiente = dynamic_cast<TCCUnionEntreDepositos*>(Plenum[LNumDeposito - 1]
							->GetCCUnionEntreDep(j))->getNumeroDeposito2();
					} else
						NumDepSiguiente = dynamic_cast<TCCUnionEntreDepositos*>(Plenum[LNumDeposito - 1]
							->GetCCUnionEntreDep(j))->getNumeroDeposito1();
				...
				if (Plenum[LNumDeposito - 1]->GetCCUnionEntreDep(j)->getTipoCC() == nmCompresor) {
					if (dynamic_cast<TCCCompresor*>(Plenum[LNumDeposito - 1]->GetCCUnionEntreDep(j))
						...
						NumDepSiguiente = dynamic_cast<TCCCompresor*>(Plenum[LNumDeposito - 1]->GetCCUnionEntreDep(j))
							->getNumeroDepositoEst();
					} else
						NumDepSiguiente = dynamic_cast<TCCCompresor*>(Plenum[LNumDeposito - 1]->GetCCUnionEntreDep(j))
							->getNumeroDepositoRot();
	...
	if (PipeStepMax) {
		for (int j = 0; j < NumberOfPipes; j++) {
			if ((Pipe[JStepMax]->getTime1() - Pipe[j]->getTime0()) < (Pipe[j]->getTime1() - Pipe[j]->getTime0())) {
				...
				Pipe[j]->PutTime1(
					Pipe[j]->getTime0() + (Pipe[JStepMax]->getTime1() - Pipe[JStepMax]->getTime0()) / pow(2., i));
			}
			...
			for (int i = -1; i < 2; i++) {
				if (i == -1) {
					...
					if (BC[OneDEnd - 1]->getTipoCC() == nmVolumetricCompressor) {
						...
						dynamic_cast<TCCCompresor*>(BC[OneDEnd - 1])->ObtencionValoresInstantaneos(Theta,
					...
					if (Plenum[NumDepInicial - 1]->getTipoDeposito() == nmDepVolCte
						...
						for (int i = 0; i < Plenum[NumDepInicial - 1]->getNUnionesED(); i++) {
							if (Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i)->getTipoCC() == nmCompresor) {
								if (dynamic_cast<TCCCompresor*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i))
									->getCompressor()->getModeloCompresor() == nmCompPlenums
									&& dynamic_cast<TCCCompresor*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i))
									...
									if (dynamic_cast<TCCCompresor*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i))
										...
										NumDepSiguiente = dynamic_cast<TCCCompresor*>(Plenum[NumDepInicial - 1]
											->GetCCUnionEntreDep(i))->getNumeroDepositoEst();
									} else
										NumDepSiguiente = dynamic_cast<TCCCompresor*>(Plenum[NumDepInicial - 1]
											->GetCCUnionEntreDep(i))->getNumeroDepositoRot();
							...
							if (Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i)->getTipoCC() == nmUnionEntreDepositos
								...
								if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(
									...
									NumDepSiguiente = dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]
										->GetCCUnionEntreDep(i))->getNumeroDeposito2();
								} else
									NumDepSiguiente = dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]
										->GetCCUnionEntreDep(i))->getNumeroDeposito1();

								for (int j = 0; j < Plenum[NumDepSiguiente - 1]->getNUnionesED(); j++) {
									if (Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(j)->getTipoCC()
										== nmCompresor) {
										if (dynamic_cast<TCCCompresor*>(Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(
											j))->getNumeroCC()
											== dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]
												->GetCCUnionEntreDep(i))->getNumeroCC()) {
											indiceUED = j;
										}
									} else if (Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(j)->getTipoCC()
										== nmUnionEntreDepositos) {
										if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepSiguiente - 1]
											->GetCCUnionEntreDep(j))->getNumeroCC()
											== dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]
												->GetCCUnionEntreDep(i))->getNumeroCC()) {
											indiceUED = j;
										}
								...
								if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepSiguiente - 1]
									->GetCCUnionEntreDep(indiceUED))->getValvula()->getCDTubVol() == 0
									&& dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepSiguiente - 1]
										->GetCCUnionEntreDep(indiceUED))->getValvula()->getCDVolTub() == 0) {
								...
								if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(
									...
									if (EngineBlock) {
										dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]
											->GetCCUnionEntreDep(i))->CalculaCoeficientesDescarga(TiempoActual,
								...
								dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i))
									->PutInstanteCalculo(TiempoActual);
								Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i)->CalculaCondicionContorno(
									TiempoActual);
								dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i))
								...
								dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepInicial - 1]->GetCCUnionEntreDep(i))
									->getValvula()->AcumulaCDMedio(TiempoActual);
	...
	try {
		...
		for (int i = 0; i < Plenum[NumDeposito - 1]->getNUnionesED(); i++) {
			if (Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i)->getTipoCC() == nmCompresor) {
				if (dynamic_cast<TCCCompresor*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))->getCompressor()
					->getModeloCompresor() == nmCompPlenums
					&& dynamic_cast<TCCCompresor*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))->getInstanteCalculo()
						!= TiempoActual) {

					/* Aqui se determina el siguiente deposito en el que se continuara la busqueda */
					if (dynamic_cast<TCCCompresor*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
						->getNumeroDepositoRot() == NumDeposito) {
						NumDepSiguiente = dynamic_cast<TCCCompresor*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
							->getNumeroDepositoEst();
					} else
						NumDepSiguiente = dynamic_cast<TCCCompresor*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
							->getNumeroDepositoRot();
					...
					dynamic_cast<TCCCompresor*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))->PutInstanteCalculo(
						TiempoActual);
				}
			}
			if (Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i)->getTipoCC() == nmUnionEntreDepositos
				&& dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
					->getInstanteCalculo() != TiempoActual) {

				/* Aqui se determina el siguiente deposito en el que se continuara la busqueda */
				if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
				...
				for (int j = 0; j < Plenum[NumDepSiguiente - 1]->getNUnionesED(); j++) {
					if (Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(j)->getTipoCC() == nmCompresor) {
						...
						== nmUnionEntreDepositos) {
						if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(j))
							->getNumeroCC()
							== dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
								->getNumeroCC()) {
							indiceUED = j;
				...
				if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(indiceUED))
					->getValvula()->getCDTubVol() == 0
					&& dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDepSiguiente - 1]->GetCCUnionEntreDep(indiceUED))
				...
				/* Calculo de la condicion de contorno compresor entre depositos encontrada */
				if (dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))->getValvula()
				...
				dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))
					->AcumulaResultadosMediosUED(TiempoActual);
				dynamic_cast<TCCUnionEntreDepositos*>(Plenum[NumDeposito - 1]->GetCCUnionEntreDep(i))->getValvula()
				...
				if (Engine[0]->GetCilindro(i)->getTiempoActual() < Pipe[JStepMax]->getTime0()) {
					...
					for (int j = 0; j < Engine[0]->GetCilindro(i)->getNumeroUnionesAdm(); j++) {
						dynamic_cast<TCCCilindro*>(Engine[0]->GetCilindro(i)->GetCCValvulaAdm(j))
							->ActualizaAnguloValvula(Pipe[JStepMax]->getTime0(), Engine[0]->getRegimen());
					}
					for (int j = 0; j < Engine[0]->GetCilindro(i)->getNumeroUnionesEsc(); j++) {
