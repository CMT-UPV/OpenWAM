...
void TOutputResults::ReadAverageResults(char* FileWAM, fpos_t& filepos, TTubo** Pipe,
	...
	for (int i = 0; i < NumValvesAvg; i++) {
		...
		if (BC[ValveID - 1]->getTipoCC() == nmIntakeValve || BC[ValveID - 1]->getTipoCC()
			== nmExhaustValve) {
...
void TOutputResults::HeaderAverageResults(stEspecies *SpeciesName, TCalculoExtern* EXTERN,
	bool ThereIsDLL) {
...
void TOutputResults::OutputAverageResults(double AcumulatedTime, TCalculoExtern* EXTERN,
	bool ThereIsDLL) {
			...
			if (BCWasteGate[j]->getNumeroCC() == WasteGateID) {
				...
				if (BCWasteGate[j]->getTipoCC() == nmPipeToPlenumConnection) {
					InsWasteGate.push_back
						(dynamic_cast<TWasteGate*>(dynamic_cast<TCCDeposito*>(BCWasteGate[j])
							->getValvula()));
				}
				else if (BCWasteGate[j]->getTipoCC() == nmUnionEntreDepositos) {
					InsWasteGate.push_back(dynamic_cast<TWasteGate*>
						(dynamic_cast<TCCUnionEntreDepositos*>(BCWasteGate[j])->getValvula()));
				}
			...
			if (BCReedValve[j]->getNumeroCC() == ReedID) {
				...
				if (BCReedValve[j]->getTipoCC() == nmPipeToPlenumConnection) {
					InsReedValve.push_back
						(dynamic_cast<TLamina*>(dynamic_cast<TCCDeposito*>(BCReedValve[j])
							->getValvula()));
				}
				else if (BCReedValve[j]->getTipoCC() == nmUnionEntreDepositos) {
					InsReedValve.push_back(dynamic_cast<TLamina*>
						(dynamic_cast<TCCUnionEntreDepositos*>(BCReedValve[j])->getValvula()));
				}
...
void TOutputResults::HeaderSpaceTimeResults(double thmax, double grmax, double agincr,
	int SpeciesNumber)
				...
				for (unsigned int j = 0; j < STPipe.size(); ++j) {
					for (int k = 0; k < STPipe[j]->getNin(); ++k) {
						double temp = pow2(STPipe[j]->GetAsonido(k) * ARef) /
							(STPipe[j]->GetGamma(k) * STPipe[j]->GetRMezcla(k)) - 273.;
				...
				for (unsigned int j = 0; j < STPipe.size(); ++j) {
					for (int k = 0; k < STPipe[j]->getNin(); ++k) {
						if (STPipe[j]->getFormulacionLeyes() == nmConArea) {
							...
							double massflow = STPipe[j]->GetPresion(k) * 1e5 / pow2
								(STPipe[j]->GetAsonido(k) * ARef) * STPipe[j]->GetGamma(k)
								* STPipe[j]->GetVelocidad(k) * ARef * STPipe[j]->GetDiametro(k)
								* Pi / 4.;
					...
					for (unsigned int j = 0; j < STPipe.size(); ++j) {
						for (int k = 0; k < STPipe[j]->getNin(); ++k) {
							if (STPipe[j]->getFormulacionLeyes() == nmConArea) {
								...
								massflow = STPipe[j]->GetPresion(k) * 1e5 / pow2
									(STPipe[j]->GetAsonido(k) * ARef) * STPipe[j]->GetGamma(k)
									* STPipe[j]->GetVelocidad(k) * ARef * STPipe[j]->GetDiametro
									(k) * Pi / 4.;
							}
...
void TOutputResults::HeaderInstantaneousResults(TCalculoExtern *EXTERN, bool ThereIsDLL,
...
void TOutputResults::OutputInstantaneousResults(TCalculoExtern *EXTERN, bool ThereIsDLL,
...
void TOutputResults::WriteInstantaneous(bool EngineBlock, double Angle, double AngStep,
