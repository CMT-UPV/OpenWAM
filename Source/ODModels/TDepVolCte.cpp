...
TDepVolCte::TDepVolCte(int i, nmTipoCalculoEspecies SpeciesModel,
		int numeroespecies, nmCalculoGamma GammaCalculation, bool ThereIsEGR) :
		TDepVolCteBase(i, nmDepVolCte, SpeciesModel, numeroespecies,
		...
		while (!Converge) {
			...
			for (int i = 0; i < FNumeroUnionesED; i++) {
				...
				if (FCCUnionEntreDep[i]->getTipoCC() == nmUnionEntreDepositos) {
					...
					if (FirstStep) {
						...
						for (int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
							FMasaEspecie[j] +=
									FCCUnionEntreDep[i]->GetFraccionMasicaEspecie(
											j) * m;
						}
					}
					if (g > 0) {
			...
			if (FHayCompresor) {
				...
				if (FirstStep) {
					MasaEntrante += m;
					for (int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
						FMasaEspecie[j] += FCompresor->GetFraccionMasicaEspecie(
								j) * m;
					}
				}
				if (g > 0) {
			...
			Heat = FHeatPower * DeltaT
					* (MTemp + FGamma / (pow2(Asonido1 * __cons::ARef) * FMasa)) / 2.;
