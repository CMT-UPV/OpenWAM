...
TVenturi::TVenturi(int i, nmTipoCalculoEspecies SpeciesModel, int numeroespecies,
	nmCalculoGamma GammaCalculation, bool ThereIsEGR) : TDepVolCteBase(i, nmVenturi, SpeciesModel,
	numeroespecies, GammaCalculation, ThereIsEGR) {
...
void TVenturi::LeeDatosVenturiXML(xml_node node_venturi) {
    try {
        FNodoEntrada = GetAttributeAsInt(node_venturi,"InputNode");
        FNodoSalida = GetAttributeAsInt(node_venturi,"OutputNode");
        FNodoLateral = GetAttributeAsInt(node_venturi,"LateralNode");

        FRelacionSecciones = GetAttributeAsDouble(node_venturi,"AreaRatio");
...
void TVenturi::AsignaEntradaSalidaLateralCC() {
	try {
		...
		std::cout << "ERROR: TVenturi::AsignaEntradaSalidaLateralCC en el venturi " << FNumeroVenturi <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
	...
	try {
		...
		for (int i = 0; i < nvars; ++i) {
			...
			case 7:
				...
				break;
			default:
				std::cout << "Resultados instantaneos en venturi " << FNumeroVenturi <<
		...
		std::cout << "ERROR: TVenturi::LeeResultadosInstantVenturi en el venturi: " << FNumeroVenturi <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
...
void TVenturi::CabeceraResultadosInstantVenturi(stringstream& insoutput) {
	try {
		...
		if (FResInstantVenturi.PresionEntrada) {
			Label = "\t" + PutLabel(521) + IntToStr(FNumeroVenturi) + PutLabel(908);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.PresionGarganta) {
			Label = "\t" + PutLabel(522) + IntToStr(FNumeroVenturi) + PutLabel(908);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.MachEntrada) {
			Label = "\t" + PutLabel(523) + IntToStr(FNumeroVenturi) + PutLabel(901);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.MachGarganta) {
			Label = "\t" + PutLabel(524) + IntToStr(FNumeroVenturi) + PutLabel(901);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.VelEntrada) {
			Label = "\t" + PutLabel(525) + IntToStr(FNumeroVenturi) + PutLabel(909);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.VelLateral) {
			Label = "\t" + PutLabel(526) + IntToStr(FNumeroVenturi) + PutLabel(909);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.GastoEntrada) {
			Label = "\t" + PutLabel(527) + IntToStr(FNumeroVenturi) + PutLabel(904);
			insoutput << Label.c_str();
		}
		if (FResInstantVenturi.GastoLateral) {
			Label = "\t" + PutLabel(528) + IntToStr(FNumeroVenturi) + PutLabel(904);
			insoutput << Label.c_str();
		}
		...
		std::cout << "ERROR: TVenturi::CabeceraResultadosInstantVenturi en el venturi: " <<
...
void TVenturi::ImprimeResultadosInstantVenturi(stringstream& insoutput) {
	try {
		...
		std::cout << "ERROR: TVenturi::ImprimeResultadosInstantVenturi en el venturi: " <<
			FNumeroVenturi << std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
...
void TVenturi::ReadAverageResultsVenturi(const char *FileWAM, fpos_t &filepos) {
	...
	try {
		...
		std::cout << "ERROR: TVenturi::ReadAverageResultsVenturi en el venturi: " << FNumeroVenturi <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
...
void TVenturi::HeaderAverageResultsVenturi(stringstream& medoutput) {
	try {
		...
		std::cout << "ERROR: TVenturi::HeaderAverageResultsVenturi en el venturi: " << FNumeroVenturi <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
	...
	try {
		...
		if (FResMediosVenturi.PresionGarganta) {
			FResMediosVenturi.PresionGargantaSUM +=
				(FPressure / pow(1 + FGamma1 / 2 * pow(dynamic_cast<TCCDeposito*>(FCCLateral)
						->getMachVenturi(), 2.), FGamma / FGamma1)) * Delta;
		}
		if (FResMediosVenturi.MachEntrada) {
			FResMediosVenturi.MachEntradaSUM += fabs
				(dynamic_cast<TCCDeposito*>(FCCEntrada)->getMachVenturi()) * Delta;
		}
		if (FResMediosVenturi.MachGarganta) {
			FResMediosVenturi.MachGargantaSUM += fabs
				(dynamic_cast<TCCDeposito*>(FCCLateral)->getMachVenturi()) * Delta;
		}
		if (FResMediosVenturi.VelEntrada) {
			if (dynamic_cast<TCCDeposito*>(FCCEntrada)->getSentidoFlujo() == nmEntrante) {
				SentidoEntrada = 1;
			...
			FResMediosVenturi.VelEntradaSUM += SentidoEntrada * ARef *
				Delta*dynamic_cast<TCCDeposito*>(FCCEntrada)->getVelocity();
		}
		if (FResMediosVenturi.VelLateral) {
			if (dynamic_cast<TCCDeposito*>(FCCLateral)->getSentidoFlujo() == nmEntrante) {
				SentidoLateral = 1;
			...
			FResMediosVenturi.VelLateralSUM += SentidoLateral * ARef *
				Delta*dynamic_cast<TCCDeposito*>(FCCLateral)->getVelocity();

		}
		if (FResMediosVenturi.GastoEntrada) {
			FResMediosVenturi.GastoEntradaSUM += -Delta*dynamic_cast<TCCDeposito*>(FCCEntrada)
				->getMassflow();
		}
		if (FResMediosVenturi.GastoLateral) {
			FResMediosVenturi.GastoLateralSUM += -Delta*dynamic_cast<TCCDeposito*>(FCCLateral)
				->getMassflow();
		}
		...
		std::cout << "ERROR: TVenturi::AcumulaResultadosMediosVenturi en el venturi: " <<
			FNumeroVenturi << std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
...
void TVenturi::ResultadosMediosVenturi() {
	try {
		if (FResMediosVenturi.PresionEntrada) {
			FResMediosVenturi.PresionEntradaMED = FResMediosVenturi.PresionEntradaSUM /
				FResMediosVenturi.TiempoSUM;
			FResMediosVenturi.PresionEntradaSUM = 0.;
		}
		if (FResMediosVenturi.PresionGarganta) {
			FResMediosVenturi.PresionGargantaMED = FResMediosVenturi.PresionGargantaSUM /
				FResMediosVenturi.TiempoSUM;
			FResMediosVenturi.PresionGargantaSUM = 0.;
		}
		if (FResMediosVenturi.MachEntrada) {
		...
		if (FResMediosVenturi.MachGarganta) {
			FResMediosVenturi.MachGargantaMED = FResMediosVenturi.MachGargantaSUM /
				FResMediosVenturi.TiempoSUM;
			FResMediosVenturi.MachGargantaSUM = 0.;
		}
		if (FResMediosVenturi.VelEntrada) {
		...
		if (FResMediosVenturi.GastoEntrada) {
			FResMediosVenturi.GastoEntradaMED = FResMediosVenturi.GastoEntradaSUM /
				FResMediosVenturi.TiempoSUM;
			FResMediosVenturi.GastoEntradaSUM = 0.;
		}
		if (FResMediosVenturi.GastoLateral) {
			FResMediosVenturi.GastoLateralMED = FResMediosVenturi.GastoLateralSUM /
				FResMediosVenturi.TiempoSUM;
			FResMediosVenturi.GastoLateralSUM = 0.;
		}
		FResMediosVenturi.TiempoSUM = 0;
		...
		std::cout << "ERROR: TVenturi::ResultadosMediosVenturi en el venturi: " << FNumeroVenturi <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
...
void TVenturi::ImprimeResultadosMediosVenturi(stringstream& medoutput) {
	try {
		...
		std::cout << "ERROR: TVenturi::ImprimeResultadosMediosVenturi en el venturi: " <<
			FNumeroVenturi << std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
