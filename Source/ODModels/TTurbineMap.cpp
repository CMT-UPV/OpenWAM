...
void TTurbineMap::LoadTurbineMapXML(xml_node node_turb) {
	...
	double Diam1 = GetXMLLength(node_turb, "WheelDiameter");
	double Diam2 = GetXMLLength(node_turb, "WheelDiameterOut");
	double Diam3 = GetXMLLength(node_turb, "NutDiameter");
	double Diam4 = GetXMLLength(node_turb, "InletDiameter");
	double CriticalAngle = GetXMLAngle(node_turb, "CriticalAngle");
	...
	for (xml_node node_map = GetNodeChild(node_turb, "Trb_TurbineMap");
		...
		ang = GetXMLAngle(node_map, "BladeAngle");
		FTurbPosition[id].ReadTurbinPositionXML(node_map);
		if (ang > CriticalAngle)
			CalculaGR = false;
		else
			CalculaGR = true;
		FTurbPosition[id].EffectiveArea(Area, CalculaGR, Diam1, Diam2, Diam3,
				n_limit);
		FTurbPosition[id].SearchMapLimits();
