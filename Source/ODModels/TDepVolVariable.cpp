...
void TDepVolVariable::LeeDatosDepVolVariable(const char *FileWAM,
...
void TDepVolVariable::LeeDatosDepVolVariableXML(xml_node node_plenum,
	...
	try {
		...
		FDiametro = GetXMLLength(node_vblvol, "Diameter");
		FRelCompre = GetAttributeAsDouble(node_vblvol, "ConnectionRod");
		FDesfase = GetXMLAngle(node_vblvol, "PhaseShift");
		FDescentramiento = GetXMLLength(node_vblvol, "Eccentricity");

		const char_t *ControlRegimen =
				node_vblvol.attribute("SpeedControl").value();
		...
		if (FControlRegimen == nmPropio) {
			FRegimen = GetXMLRotationalSpeed(node_vblvol, "Speed");
			FRelacionVelocidades = 1.;
		} else if (FControlRegimen == nmMotor && HayMotor) {
			FRelacionVelocidades = GetAttributeAsDouble(node_vblvol,
					"SpeedRatio");
		} else {
			std::cout
					<< "ERROR: TDepVolVariable::LeeDatosIniciales Lectura del Control del Regimen erronea "
