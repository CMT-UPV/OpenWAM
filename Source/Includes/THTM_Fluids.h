...
struct stHTM_Fluid {
	...
	double Property(double T, dVector A) {
		return A[0] + A[1] * T + A[2] * pow2(T) + A[3] * pow3(T) + A[4] * log
		(T) + A[5] / T + A[6] / pow2(T);
	};
...
struct stBurntAirWater : stHTM_Fluid {
	...
	stBurntAirWater() : stHTM_Fluid() {
		X.resize(2);
		X[0] = 0.;
		X[1] = 0.;
	}

	stBurntAirWater(dVector _X) : stHTM_Fluid() {
		X.resize(2);
		X[0] = _X[0];
		X[1] = _X[1];
	}
