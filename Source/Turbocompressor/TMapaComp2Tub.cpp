...
{
if(FGastoRelComp1!=NULL) delete[] FGastoRelComp1;
FGastoRelComp1=NULL;
if(FGastoBombeo!=NULL) delete[] FGastoBombeo;
FGastoBombeo=NULL;
if(FRelCompBombeo!=NULL) delete[] FRelCompBombeo;
FRelCompBombeo=NULL;
if(FRelComp!=NULL){
...
FRend=NULL;
if(FSpl!=NULL) delete[] FSpl;
FSpl=NULL;
if(FOrtp!=NULL) delete[] FOrtp;
FOrtp=NULL;
if(FCoefSplBombeo!=NULL) delete[] FCoefSplBombeo;
FCoefSplBombeo=NULL;
if(FCoefSplRC!=NULL) delete[] FCoefSplRC;
FCoefSplRC=NULL;
if(FRegimenCurva!=NULL) delete[] FRegimenCurva;
FRegimenCurva=NULL;
if(FNumCurvasRen!=NULL) delete[] FNumCurvasRen;
FNumCurvasRen=NULL;
if(FNumCurvasRenAd!=NULL) delete[] FNumCurvasRenAd;
FNumCurvasRenAd=NULL;
if(FCoefbSup!=NULL){
     for(int i=0;i<FNumCurvasReg;i++){
          delete[] FCoefbSup[i];
...
FCoefSplRend=NULL;
if(Fnegmas!=NULL) delete[] Fnegmas;
Fnegmas=NULL;
if(Fnegrc!=NULL) delete[] Fnegrc;
Fnegrc=NULL;
if(Fsolneg!=NULL) delete[] Fsolneg;
Fsolneg=NULL;
...
FRelCompNuevo=NULL;
if(FGastoCompNuevo!=NULL) delete[] FGastoCompNuevo;
FGastoCompNuevo=NULL;
}
...
void TMapaComp2Tub::LeeMapa(FILE *fich,double radtip, double radhub,double radrodete)
{
double *W=NULL;
try
{
...
for(int i=0;i<FNumCurvasReg;i++){
	 if(FGastoRend[i][0]<FGastoBombeo[i+1]*1.001 && FGastoRend[i][0]>FGastoBombeo[i+1]*0.999){
		  ...
		  RendEnBombeo=true;
		  std::cout << "INFO: Surge limit for " << FRegimenCurva[i] << " rpm iso-speed line has an efficiency value deffined" << std::endl;
	 }else{
          FNumCurvasRenAd[i]=FNumCurvasRen[i]+2;
		  RendEnBombeo=false;
		  std::cout << "INFO: Surge limit for " << FRegimenCurva[i] << " rpm iso-speed line does not have an efficiency value deffined" << std::endl;
	 }
...
if(W!=NULL)delete[] W;
throw Exception("ERROR: LeeMapa en el compresor: "+AnsiString(FNumeroCompresor)+N.Message.c_str());
}
}
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: Spline en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: EvaluaSpline en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
double TMapaComp2Tub::EvaluaSpline(double punto,int n,std::vector<double> x,std::vector<double> y,std::vector<double> sol)
{
try
{
int k=0;
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: EvaluaRCSpline en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
void TMapaComp2Tub::PolOrtogonal(int nterms,int npoint,double *ma,double *rd,double *w,double *b,
     double *c,double *d)
{
double p=0;
try
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: PolOrtogonal en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
if(FRegComp>=FRegMax){
	FGastoBombeoX=FGastoBombeo[FCurvInf+1]+(FGastoBombeo[FCurvInf+1]-FGastoBombeo[FCurvInf])/
		  FIncReg*(FRegComp-FRegMax);

	FRelCompBombeoX=FRelCompBombeo[FCurvInf+1]+(FRelCompBombeo[FCurvInf+1]-FRelCompBombeo[FCurvInf])/
		  FIncReg*(FRegComp-FRegMax);

	FGastoRelComp1X=FGastoRelComp1[FCurvInf]+(FGastoRelComp1[FCurvInf]-FGastoRelComp1[FCurvInf-1])/
		  FIncReg*(FRegComp-FRegMax);

}else if(FRegComp<FRegMin){
	FGastoBombeoX=FGastoBombeo[FCurvInf+1]-(FGastoBombeo[FCurvInf+2]-FGastoBombeo[FCurvInf+1])/
		  FIncReg*(FRegMin-FRegComp);

	FRelCompBombeoX=EvaluaHermite(FGastoBombeoX,FNumCurvasReg+1,FGastoBombeo,FRelCompBombeo,
		FCoefSplBombeo);

	 FGastoRelComp1X=FGastoRelComp1[FCurvInf]-(FGastoRelComp1[FCurvInf+1]-FGastoRelComp1[FCurvInf])/
		  FIncReg*(FRegMin-FRegComp);
}else{
	FGastoBombeoX=FGastoBombeo[FCurvInf+1]+(FGastoBombeo[FCurvInf+2]-FGastoBombeo[FCurvInf+1])*
		  DeltaN;
...
if(FGastoBombeoX<0) FGastoBombeoX=0.;
if(FGastoRelComp1X<FGastoBombeoX) FGastoRelComp1X=FGastoBombeoX+1e-6;
...
if(FRegComp<FRegMin){
	for(int j=0;j<FNumPuntos;++j){
		...
		FRelCompInt[j]=FRelCompNuevo[FCurvInf][j]-(FRelCompNuevo[FCurvInf+1][j]-FRelCompNuevo[FCurvInf][j])/FIncReg*
			(FRegMin-FRegComp);
		if(FRelCompInt[j]<1.) FRelCompInt[j]=1.;
	}
}else{
	for(int j=0;j<FNumPuntos;++j){
		  //FGastoInt[j]=FGastoMin+FIncGasto*j;
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: InterpolaMapa en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: EvaluaRendimiento en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
if(MasaAire<FGastoBombeoX){
	 ...
	 RendInf=EvaluaHermite(AireAdim,FNumCurvasRenAd[index],FGastoAdim[index],
		  FRendAdim[FCurvInf],FCoefSplRend[FCurvInf]);

	 //RendInf=InterpolaLineal(AireAdim,FNumCurvasRenAd[index],FGastoAdim[index],
	 //	  FRendAdim[FCurvInf]);

	 RendSup=EvaluaHermite(AireAdim,FNumCurvasRenAd[index+1],FGastoAdim[index+1],
		  FRendAdim[index+1],FCoefSplRend[index+1]);
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: EvaluaRendSplines en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: GetRelCompInt en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: GetGastoInt en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: BuscaRegimen en el compresor: "/* +AnsiString(FNumeroCompresor)+N.Message.c_str()*/);
}
}
...
for (int i=0; i<FNumCurvasReg; ++i) {
	 ...
	 for (int j=0;j<FNumPuntosGastoNuevo;++j){
		  if (FGastoMin+j*FIncGasto < FGastoBombeo[i]){
               ...
               if (FGastoMin+j*FIncGasto>FIncGasto/10)++cont;
		  }else {
			...
			for (int j = 0; j < 21; j++) {
				...
				if (FlowMass>FGastoBombeo[i]) {
					if(FlowMass<FGastoRelComp1[i]){
						FRelComp[i][j]=EvaluaHermite(FlowMass,FGTMass[i].size(),
							FGTMass[i],FGTPres[i],FGTCoefCR[i]);
					}else{
						FRelComp[i][j]=1.;
                    }
