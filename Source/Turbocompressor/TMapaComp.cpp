...
TMapaComp::TMapaComp(int i):TCompressorMap()
{
FNumeroCompresor=i;
...
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception("ERROR: EvaluaRCSpline en el compresor: " +AnsiString(FNumeroCompresor)+N.Message);
}
}
...
if(MasaAire<FGastoBombeoX){
     ...
     RendInf=EvaluaHermite(AireAdim,FNumCurvasRenAd[FCurvInf],FGastoAdim[FCurvInf],
