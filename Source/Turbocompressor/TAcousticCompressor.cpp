...
TAcousticCompressor::TAcousticCompressor(int InletPipeID,
	int VoluteID, int OutletPipeID, int RotorVolumeID, int StatorVolumeID) {
...
double TAcousticCompressor::P10() {
	...
	double p0 = p * pow(1 + (g - 1) / 2 * pow2(v / a), 2 * g / (g - 1));
...
double TAcousticCompressor::EFCorrector(double rcorr, double rorig) {
	...
	(pow(rorig * rcorr, (g - 1) / g) - 1) / (pow(rorig, (g - 1) / g) - 1);
