...
/* NUEVO- PROGRAMACI�N DE LAS EXPRESIONES NECESARIAS PARA
EL M�TODO DE BRENT:
...
struct stExpansion {
	...
	/*Variables locales que almacenar�n la inforamci�n pasada por funci�n */

	double CCS; // Variable local asociada a la caracteristica conocida en 1 (salida del tubo) y que se pasa por funci�n//
	...
	/* Variables locales internas, si no se quiere acceder despu�s a la
	informaci�n que contienen, es preferible definirlas directamente dentro del
	operador */
	...
	stExpansion(const double iCCS, const double iCCE, const double R_E, const double R_A, const double iGam)
		...
		Ga3 = (Gam - 1) / 2;
		/* Definici�n de aquellas variables internas que se usaran en el operador, pero que no se requiere su acceso, aqui o dentro del operador */
		Ga2 = Gam + 1;
	}
...
struct stContraction {
	...
	/* Variables locales para almacenar la informaci�n pasada por funcion */
	...
	double rel_entropia; // Variable l.a a la relacion de entropia pasada por funcion//
	double rel_area; // Variable l.a a la relaci�n de area, pasada por funcion//
	double Gam; // Variable l.a a gamma, pasado por funcion//

	/* Variables locales internas, que permitir�n el acceso. Si no se requiere acceso
	...
	/* CONSTRUCTOR: Se recibe la informaci�n por funcion en variables ficticias y se
...
struct stPerdPresAd {
	...
	/* Definir las variables locales donde se almacenar� la informacion que se pase
	...
	double FK; // Variable l.a. al par�metro K pasado por funcion//
	double Gam; // Variable l.a. a Gamma pasado por funcion//
...
struct stPerdPresAdL {
	...
	double operator()(const double A1) {
		...
		U1 = (CC1 - A1) / Ga3; // C�lculo de la velocidad  saliente.//
		double b1 = -FK * fabs(U1) / pow2(A1) + 1.;
		b1 += 1e-14; // Resoluci�n ecuaci�n de 2� grado y c�lculo de la velocidad entrante.OJO que he cambiado la f�rmula.Pedro.//
		// Resoluci�n ecuaci�n de 2� grado y c�lculo de la velocidad entrante.//
		double a = Ga3;
		double b = b1 * pow2(A1);
		double c = -(pow2(A1) + Ga3 * pow2(U1)) * pow2(U1);
		...
		// C�lculo de la velocidad del sonido entrante.
...
struct stComprVol {
	...
	y que almacenar�n la informaci�n de las variables pasadas a la funcion
	desde el programa, LA INFORMACI�N SE ASOCIAR� EN EL CONSTRUCTOR. AQUI
	...
	y que son necesarias para el c�lculo del operador (tipo de variable y nombre ficticio) y a continuaci�n
	SE ASOCIAN a las variables locales que almacenaran la informaci�n pasada */
