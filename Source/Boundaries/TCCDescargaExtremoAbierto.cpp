...
void TCCDescargaExtremoAbierto::AsignAmbientConditions(double Tamb, double Pamb, double *AtmosphericComposition) {
	...
	if (FCalculoEspecies == nmCalculoCompleto) {
		...
		CvMezclaDep = CalculoSimpleCvMezcla(__units::degCToK(FTemperaturaDep), FComposicion[0], 0, FCalculoGamma,
			nmMEP);
	...
	try {
		...
		if (FTipoDescarga == nmDescargaAtmosfera) {  // DESCARGA A LA ATMOSFERA
			...
			if (FCalculoEspecies == nmCalculoCompleto) {
				...
				RMezclaDep = CalculoSimpleRMezcla(FComposicion[0], 0, FCalculoGamma, nmMEP);
				CvMezclaDep = CalculoSimpleCvMezcla(__units::degCToK(FTemperaturaDep), FComposicion[0], 0,
					FCalculoGamma, nmMEP);
				GammaDep = CalculoSimpleGamma(RMezclaDep, CvMezclaDep, FCalculoGamma);
		...
		std::cout << "ERROR: TCCDescargaExtremoAbierto::LeeDescargaExtremoAbierto en la condicion de contorno: "
			<< FNumeroCC << std::endl;
...
void TCCDescargaExtremoAbierto::CalculaCondicionContorno(double Time) {
	try {
		...
		std::cout << "ERROR: TCCDescargaExtremoAbierto::CalculaCondicionesContorno en la condicion de contorno: "
			<< FNumeroCC << std::endl;
		std::cout << "Tipo de error: " << N.what() << std::endl;
		throw Exception(N.what());
	}
