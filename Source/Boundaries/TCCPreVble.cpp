...
TCCPreVble::TCCPreVble(nmTypeBC TipoCC,int numCC,nmTipoCalculoEspecies SpeciesModel,
             ...
             TCondicionContorno(TipoCC,numCC,SpeciesModel,numeroespecies,GammaCalculation,
             ThereIsEGR)
{
...
void TCCPreVble::ReadBoundaryData(char *FileWAM,fpos_t &filepos,int NumberOfPipes,TTubo **Pipe,int nDPF, TDPF **DPF)
{
try
{
int i=0;
...
{
std::cout << "ERROR: TCCPreVble::LecturaPulso en la condici�n de contorno: " << FNumeroCC <<  std::endl;
...
if((FTuboExtremo[0].TipoExtremo==nmIzquierda && FTuboExtremo[0].Pipe->GetVelocidad(FNodoFin)>=0)
     || (FTuboExtremo[0].TipoExtremo==nmIzquierda && FTuboExtremo[0].Pipe->GetVelocidad(FNodoFin)<=0)){
...
{
std::cout << "ERROR: TCCPreVble::CalculaCondicionesContorno en la condici�n de contorno: " << FNumeroCC << std::endl;
std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
throw Exception(N.Message.c_str());
}
}
