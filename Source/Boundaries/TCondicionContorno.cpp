...
void TCondicionContorno::CalculaCaracteristicas(double Time)
{
	...
	double dt=Time-FTuboExtremo[i].Pipe->getTime0();
		if(FTuboExtremo[i].TipoExtremo==nmRight){
			...
			FTuboExtremo[i].Landa=FTuboExtremo[i].Pipe->Interpola_Caracteristica(FTuboExtremo[i].Entropia,
				-1,nin,dt);
		}else{
			...
			FTuboExtremo[i].Beta=FTuboExtremo[i].Pipe->Interpola_Caracteristica(FTuboExtremo[i].Entropia,
				1,0,dt);
		}
	}
}
