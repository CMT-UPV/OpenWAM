...
if(FNodoFin!=NULL) delete[] FNodoFin;
if(FIndiceCC!=NULL) delete[] FIndiceCC;
if(*FCC!=NULL) delete[] FCC;
if(*FCD!=NULL) delete[] FCD;
if(FNumeroTubo!=NULL) delete[] FNumeroTubo;
}
...
if (flujo < .999995) {  /* Flujo de 0 (Saliente) a 1 (Entrante) */
	...
	if(FTipoPP==nmPPLineal){ /* P�rdida lineal */
		...
		stPerdPresAdL PPAL (*FCC[0],*FCC[1],FK,FGamma,FRelacionEntropia,ARef);
	...
	if(PPA.U1 * PPA.U2 < 0){
        ...
        if(FHayEGR) FFraccionMasicaEspecie[FNumeroEspecies-1]=FTuboExtremo[0].Pipe->GetFraccionMasicaCC(FIndiceCC[0],FNumeroEspecies-1);
		...
		if (FTipoPP==nmPPLineal){ /* Linear pressure loss */
			...
			ed= *FCC[1];
			   ...
			   stPerdPresAdL PPAL (*FCC[1],*FCC[0],FK,FGamma,FRelacionEntropia,ARef);
			   vel_sonido_Out= FindRoot(PPAL,ei,ed);
			   vel_sonido_In=PPAL.A2;
			   vel_In=PPAL.U1;
			   vel_Out=PPAL.U2;
        ...
        FFraccionMasicaEspecie[FNumeroEspecies-2]=1.-FraccionMasicaAcum;
        if(FHayEGR) FFraccionMasicaEspecie[FNumeroEspecies-1]=FTuboExtremo[1].Pipe->GetFraccionMasicaCC(FIndiceCC[1],FNumeroEspecies-1);
