...
TCCExternalConnection::TCCExternalConnection(nmTypeBC TipoCC, int numCC,
...
void TCCExternalConnection::UpdateCurrentExternalProperties(double U0,
		double U1, double T0, double T1, double P0, double P1, double t) {
...
void TCCExternalConnection::AsignGeometricalData(double D0, double D1,
		double deltaX) {

	FDExt[0] = D0;
	FDExt[1] = D1;
	FAExt[0] = D0 * D0 * __CTE.Pi_4;
	FAExt[1] = D1 * D1 * __CTE.Pi_4;
...
void TCCExternalConnection::ExternalCharacteristics(double Time) {
	...
	if (FUExt[0] > 0) {
		...
		FA_AExt = (AA0 * (FDeltaX - x) + AA1 * x) / FDeltaX / __CTE.ARef;

	} else {
		FA_AExt = AA0 / __CTE.ARef;
	}
	...
	double deltaEnt = Ax * (AA0 - FA_AExt * __CTE.ARef) / FA_AExt / __CTE.ARef;

	FK_CExt = (Ax + (g - 1) / 2 * Ux + deltaEnt) / __CTE.ARef;
...
void TCCExternalConnection::ReadBoundaryData(const char *FileWAM,
