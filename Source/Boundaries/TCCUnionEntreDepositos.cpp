...
switch(Origen[Valv-1]->getTypeOfValve()){
        case nmCDFijo: FValvula=new TCDFijo(dynamic_cast<TCDFijo *>(Origen[Valv-1]),i); break;
        case nmValvula4T: FValvula=new TValvula4T(dynamic_cast<TValvula4T *>(Origen[Valv-1]),i); break;
        case nmLamina: FValvula=new TLamina(dynamic_cast<TLamina *>(Origen[Valv-1]),i); break;
        case nmDiscoRotativo: FValvula=new TDiscoRotativo(dynamic_cast<TDiscoRotativo *>(Origen[Valv-1]),i); break;
        case nmLumbrera2T: FValvula=new TLumbrera(dynamic_cast<TLumbrera *>(Origen[Valv-1]),i); break;
        case nmValvulaContr: FValvula=new TValvulaContr(dynamic_cast<TValvulaContr *>(Origen[Valv-1]),i); break;
        case nmWasteGate: FValvula=new TWasteGate(dynamic_cast<TWasteGate *>(Origen[Valv-1]),i); break;
        case nmStator: FValvula=new TEstatorTurbina(dynamic_cast<TEstatorTurbina *>(Origen[Valv-1]),i); break;
        case nmRotor: FValvula=new TRotorTurbina(dynamic_cast<TRotorTurbina *>(Origen[Valv-1]),i); break;
        case nmCalcExtern: FValvula=new TCDExterno(dynamic_cast<TCDExterno *>(Origen[Valv-1]),i); break;
}
...
}else  FValvula->AsignaCRecuperacion(0.);
}
...
if(FIndependiente){
  ...
  /* if(FNumeroCC==3){
   ...
   if(FDeposito1->getPressure() > FDeposito2->getPressure()){ /* Flujo del dep�sito 1 al dep�sito 2 */
      ...
      FFraccionMasicaEspecie[FNumeroEspecies-2]=1.-FraccionMasicaAcum;
      if(FHayEGR) FFraccionMasicaEspecie[FNumeroEspecies-1]=FDeposito2->GetFraccionMasicaEspecie(FNumeroEspecies-1);
   }
...
for(int i=0;i<nvars;i++){
     fscanf(fich,"%d ",&var);
     switch(var){
          case 0: FResMediosUED.Massflow=true; break;
          default : std::cout << "Resultados medios en UED(BC) " << FNumeroCC << " no implementados " << std::endl;
     }
}
fgetpos(fich, &filepos);
fclose(fich);
...
if(FResMediosUED.Massflow){
	Label = "\t" + PutLabel(411) + AnsiString(FNumeroCC) + PutLabel(904);
