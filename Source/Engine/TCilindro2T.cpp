...
//						}
		...
		for(int i = 0; i < FNumIny; i++) {
			if(FAnguloActual > FAnguloInjeccion[i] && FAnguloAnterior <= FAnguloInjeccion[i] && FMotor->getCombustible() == nmMEC) {
		...
		if(FMotor->getCombustible() == nmMEP || FMotor->getCombustible() == nmMEC) {
			...
			FCu = __units::RPMToRad_s(FMotor->getRegimen()) * Fequis * Fctorbadmp * FKctm * pow2(
					  FMotor->getGeometria().Diametro) / (2. * FMotor->getGeometria().DiametroBowl);
			...
			if(FAnguloActual >= 90. && FAnguloActual <= 180.) {
				...
				Fh = 1.2e-2 * pow(__units::BarToPa(FPressure), 0.8) * pow(__units::degCToK(FTemperature),
						-0.53) * pow(FMotor->getGeometria().Diametro, -0.2) * pow(
						 ...
						 FMotor->getGeometria().CilindradaUnitaria / FMasaAtrapada / FRMezcla, 0.8);
			}
		}
			...
			if(FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.) {
				...
				if(FMairequem > (FMasaEspecieCicloCerrado[2])) {
					...
					if(!FSaturado) {
						...
						FMasaEspecieCicloCerrado[0] = FMasaEspecieCicloCerrado[0] - FComposicionCicloCerrado[0] * FMasaBlowBy;
						// Gases Quemados
						FMasaEspecieCicloCerrado[1] = FMasaEspecieCicloCerrado[1] - FComposicionCicloCerrado[1] * FMasaBlowBy +
													  FFuelInstant; // Combustible
					}
					...
					FMasaEspecieCicloCerrado[1] = FMasaEspecieCicloCerrado[1] - FMfquem - FComposicionCicloCerrado[1] * FMasaBlowBy +
												  FFuelInstant; // Combustible
					FMasaEspecieCicloCerrado[2] = FMasaEspecieCicloCerrado[2] - FMairequem - FComposicionCicloCerrado[2] *
		...
		if(FCicloCerrado) {
			if(FMotor->getSpeciesModel() == nmCalculoCompleto) {
				if(FMotor->getACT()) {
					...
					if(FMotor->getSpeciesNumber() == 9) {
						if(!FSaturado) {
							FMolesCombQuemado = FMfquem / (FXComb * 12.01 + FYComb * 1.01 + FZComb *
														   16); // Moles de combustible quemado en el incremento temporal calculado
						} else {
							FMolesCombQuemado = mfquefin / (FXComb * 12.01 + FYComb * 1.01 + FZComb *
															16); // Moles de combustible quemado en el incremento temporal calculado
						}
