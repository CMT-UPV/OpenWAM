...
TCilindro2T::TCilindro2T(TBloqueMotor *Engine,int nc,bool ThereIsEGR):TCilindro(Engine,ThereIsEGR)
{
...
FRegInt=FMotor->getRegimen();
			...
			if(FMotor->getACT()){
				...
				CALCULUS_OF_INJECTION_RATE(FIN,FSOP,FMFI,FSOI,FEOI,Ftest_variables[0],FCAI,
				Ftest_variables[5],FA_TASA,FB_TASA,FC_TASA,FD_TASA,Finjection_rate,
				FCAD_injection_rate);

				ACT(Fengine_parameters,Fengine_model_constants,Ftest_variables,Finjection_rate,
				FCAD_injection_rate,Fsize_inlet_inj,FIN,FSOI,FEOI,FCAI,FCAD_exit,FHRF_exit,
				FROHR_exit,Fp_cyl_exit,Fdp_da_cyl_exit,FT_cyl_exit,FH_cooler_exit,Fmean_var_exit,
				Fheat_transfer,Finjection_rate_exit,Faccum_injection_rate_exit,FSpecies_EVO);

				// Normalize total mass fraction.
				double ACT_Correction=0;
				for(int i=0;i<8;i++){
					ACT_Correction+=FSpecies_EVO[i];
				}
				for(int i=0;i<8;i++){
					FSpecies_EVO[i]=FSpecies_EVO[i]/ACT_Correction;
				}
		...
		if(FMotor->getGammaCalculation()==nmGammaConstante){
			if(FMotor->getSpeciesModel()==nmCalculoCompleto){
				if(!FCicloCerrado){
					...
					FGamma=FuncionGamma(FTemperature+273.,FComposicionCicloCerrado[0]);
				}
			}else if(FMotor->getSpeciesModel()==nmCalculoSimple){
				FGamma=FuncionGamma(FTemperature+273.,FFraccionMasicaEspecie[0]);
			}
			FRMezcla=R;
			...
			if(FMotor->getSpeciesModel()==nmCalculoCompleto){
				...
				FFraccionMasicaEspecie[2],FTemperature+273.,FMotor->getGammaCalculation());
				FGamma=CalculoCompletoGamma(FRMezcla,FCpMezcla,FMotor->getGammaCalculation());

			}else if(FMotor->getSpeciesModel()==nmCalculoSimple){

				FRMezcla=CalculoSimpleRMezcla(FFraccionMasicaEspecie[0],FMotor->getGammaCalculation());
				FCvMezcla=CalculoSimpleCvMezcla(FTemperature+273.,FFraccionMasicaEspecie[0],FMotor->getGammaCalculation());
...
FDeltaAngulo=360.*FMotor->getRegimen()/60.*FDeltaT;
		...
		if(!FCicloCerrado){
			// GASTO POR ADMISION
			for(int i=0;i<FNumeroUnionesAdm;i++){
				FMasaValvAdm=-dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getMassflow()*FDeltaT;  // signo - xq el massflow entrante se calcula negativo en la BC. As� en el cilindro ser� positivo si es entrante y viceversa.
				if(FAnguloActual-FDeltaAngulo<FDistribucion.CA && FAnguloActual > FDistribucion.CA) {     //FDistribucion.CA es el CrankAngle de cierre de la admision.
				...
				if(FHayEGR) FAcumMasaEGR+=FCCValvulaAdm[i]->GetFraccionMasicaEspecie(FMotor->getSpeciesNumber()-1)*FMasaValvAdm;
			}
			// GASTO POR ESCAPE
			for(int i=0;i<FNumeroUnionesEsc;i++){
				masavalesc=-dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getMassflow()*FDeltaT;  // signo - xq el massflow entrante se calcula negativo en la BC. As� en el cilindro ser� positivo si es entrante y viceversa.
				if(FAnguloActual-FDeltaAngulo<FDistribucion.CE && FAnguloActual>FDistribucion.CE) {
					masavalesc=masavalesc*(FDistribucion.CE-FAnguloActual+FDeltaAngulo)/FDeltaAngulo;    //FDistribucion.CE es el CrankAngle de cierre del escape.
				}
		...
		if(FPressure>FMotor->getPresionAmb() && FMotor->getGeometria().CDBlowBy>0.){
			...
			FGastoBlowBy=FMotor->getGeometria().CDBlowBy*3.5e-5*FMotor->getGeometria().Diametro*FPressure*1e5*sqrt(z/FRMezcla/(FTemperature+273.));
		}else{
			FGastoBlowBy=0.;
		}
		...
		if(FAnguloComb>FAnguloInjeccion && FAnguloComb0<FAnguloInjeccion && FMotor->getCombustible()==nmMEC){
		...
		if(FAnguloActual>FDistribucion.CA && FAnguloAnterior<=FDistribucion.CA && !FCicloCerrado){
			...
			if(FHayEGR) FFraccionMasicaEspecie[FMotor->getSpeciesNumber()-1]=1.;
		...
		// When
		if(FMotor->getCombustible()==nmMEP || FMotor->getCombustible()==nmMEC){
			...
			double condk = 6.1944e-3 + 7.3814e-5 * tcil - 1.2491e-8 * pow2(tcil);
			...
			if(FAnguloActual>=0. && FAnguloActual<=90.){
				...
				Fequis=Fratioctm+1/(pow(cosh((360.-FAnguloActual)/100.),40.)+Fratioctm/(1-Fratioctm));
			}else{
				Fequis=0.;
			}
FCu=(2.*Pi*FMotor->getRegimen()/60)*Fequis*Fctorbadmp*FKctm*pow2(FMotor->getGeometria().Diametro)/(2.*FMotor->getGeometria().DiametroBowl);
			...
			double deltaP=(FPressure-FPresionRCA*pow(FVolumenCA/FVolumen,1.36))*1e5;
			if (deltaP<0.) deltaP = 0.;
			if(FAnguloActual>=90. && FAnguloActual<=180.){         // CARRERA DE ESCAPE
				Fh=1.2e-2*pow(FPressure*1e5,0.8)*pow(FTemperature+273.,-0.53)*pow(FMotor->getGeometria().Diametro,-0.2)
*pow((6.18*FCm+0.417*FCu)*FMotor->getAjusteTranCalEsc()+Fc2*deltaP*FMotor->getGeometria().CilindradaUnitaria/FMasaAtrapada/FRMezcla,0.8);
			}else if(FAnguloActual>=180. && FAnguloActual<=270.){   // CARRERA DE ADMISION
				Fh=1.2e-2*pow(FPressure*1e5,0.8)*pow(FTemperature+273.,-0.53)*pow(FMotor->getGeometria().Diametro,-0.2)
*pow((6.18*FCm+0.417*FCu)*FMotor->getAjusteTranCalAdm()+Fc2*deltaP*FMotor->getGeometria().CilindradaUnitaria/FMasaAtrapada/FRMezcla,0.8);
			}else{  // CARRERAS DE COMPRESION Y EXPANSION
				Fh=1.2e-2*pow(FPressure*1e5,0.8)*pow(FTemperature+273.,-0.53)*pow(FMotor->getGeometria().Diametro,-0.2)
					*pow((FMotor->getWoschni().cw1*FCm+FMotor->getWoschni().cw2*FCu)+Fc2*deltaP*FMotor->getGeometria().CilindradaUnitaria/FMasaAtrapada/FRMezcla,0.8);
			}
		}



		if(FMotor->getCalculoPared()!=nmTempFija) CalculaTemperaturasPared();

		// CALOR TRANSMITIDO A LAS PAREDES
		FCalor.TransCilindro=Fh*(FTempPared[0].Cylinder-FTemperature)*4*(FVolumen/2.+FVolumen0/2.-FMotor->getGeometria().VCC)/FMotor->getGeometria().Diametro*FDeltaT;
		FCalor.TransPiston=Fh*(FTempPared[0].Piston-FTemperature)*FMotor->getGeometria().AreaPiston*FDeltaT;
		FCalor.TransCulata=Fh*(FTempPared[0].Culata-FTemperature)*FMotor->getGeometria().AreaCulata*FDeltaT;
		FCalor.TransTotal=FCalor.TransCilindro+FCalor.TransPiston+FCalor.TransCulata;
		...
		if(FCicloCerrado){
			if(FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.){
				...
				FMasaEspecieCicloCerrado[0]=FMasaEspecieCicloCerrado[0]-FComposicionCicloCerrado[0]*FMasaBlowBy; // Gases Quemados
				FMasaEspecieCicloCerrado[1]=FMasaEspecieCicloCerrado[1]+FFuelInstant;  // Combustible
				FMasaEspecieCicloCerrado[2]=FMasaEspecieCicloCerrado[2]-FComposicionCicloCerrado[2]*FMasaBlowBy;  // Aire Fresco
				...
				FCalor.Liberado=(FCalor.FQL-FCalor.FQL0)*FMfint*FMotor->getPoderCalorifico()*FMotor->getRendimientoCombustion();
				FCalor.LiberadoTotal+=FCalor.Liberado;
				if(FCalor.Liberado<0) FCalor.Liberado=0.;
				...
				if(FMairequem>(FMasaEspecieCicloCerrado[2])){
					...
					if(!FSaturado){
						...
						FCalor.Liberado = (FCalor.FQL-FCalor.FQL0)*mfquefin*FMotor->getPoderCalorifico()*FMotor->getRendimientoCombustion();

						FSaturado = true;

						FMasaEspecieCicloCerrado[0]=FMasaEspecieCicloCerrado[0]+FMairequem+mfquefin-FComposicionCicloCerrado[0]*FMasaBlowBy; // Gases Quemados
						FMasaEspecieCicloCerrado[1]=FMasaEspecieCicloCerrado[1]-mfquefin-FComposicionCicloCerrado[1]*FMasaBlowBy+FFuelInstant;  // Combustible
					...
					FMasaEspecieCicloCerrado[0]=FMasaEspecieCicloCerrado[0]+FMairequem+FMfquem-FComposicionCicloCerrado[0]*FMasaBlowBy; // Gases Quemados
					FMasaEspecieCicloCerrado[1]=FMasaEspecieCicloCerrado[1]-FMfquem-FComposicionCicloCerrado[1]*FMasaBlowBy+FFuelInstant;  // Combustible
					FMasaEspecieCicloCerrado[2]=FMasaEspecieCicloCerrado[2]-FMairequem-FComposicionCicloCerrado[2]*FMasaBlowBy;  // Aire Fresco
				}
		...
		if(FCicloCerrado){
			if(FMotor->getSpeciesModel()==nmCalculoCompleto){
				if(FMotor->getACT()){
					...
					FFraccionMasicaEspecie[0]=(FSpecies_IVC[1]*(1-FCalor.FQL)+FSpecies_EVO[1]*FCalor.FQL)*(1-Yfuel);
					FFraccionMasicaEspecie[1]=(FSpecies_IVC[2]*(1-FCalor.FQL)+FSpecies_EVO[2]*FCalor.FQL)*(1-Yfuel);
					FFraccionMasicaEspecie[2]=(FSpecies_IVC[3]*(1-FCalor.FQL)+FSpecies_EVO[3]*FCalor.FQL)*(1-Yfuel);
					FFraccionMasicaEspecie[3]=(FSpecies_IVC[7]*(1-FCalor.FQL)+FSpecies_EVO[7]*FCalor.FQL)*(1-Yfuel);
					FFraccionMasicaEspecie[4]=(FSpecies_IVC[6]*(1-FCalor.FQL)+FSpecies_EVO[6]*FCalor.FQL)*(1-Yfuel);
					FFraccionMasicaEspecie[5]=(FSpecies_IVC[4]*(1-FCalor.FQL)+FSpecies_EVO[4]*FCalor.FQL)*(1-Yfuel);
					FFraccionMasicaEspecie[6]=(FSpecies_IVC[5]*(1-FCalor.FQL)+FSpecies_EVO[5]*FCalor.FQL)*(1-Yfuel);

					if(FMotor->getSpeciesNumber()==9){
						FFraccionMasicaEspecie[3]+=Yfuel;
						FFraccionMasicaEspecie[7]=(FSpecies_IVC[0]*(1-FCalor.FQL)+
						FSpecies_EVO[0]*FCalor.FQL)*(1-Yfuel);

					}else if(FMotor->getSpeciesNumber()==10){
						FFraccionMasicaEspecie[7]=Yfuel;
						FFraccionMasicaEspecie[8]=(FSpecies_IVC[0]*(1-FCalor.FQL)+
						FSpecies_EVO[0]*FCalor.FQL)*(1-Yfuel);
					}
					for(int j=0;j<FMotor->getSpeciesNumber()-FIntEGR;j++){
					...
					if(FMotor->getSpeciesNumber()==9){
						if(!FSaturado){
							FMolesCombQuemado=FMfquem/(FXComb*12.01+FYComb*1.01+FZComb*16);     //Moles de combustible quemado en el incremento temporal calculado
						}else{
							FMolesCombQuemado=mfquefin/(FXComb*12.01+FYComb*1.01+FZComb*16);     //Moles de combustible quemado en el incremento temporal calculado
						}
						FMasaO2Reactivos=FMolesCombQuemado*(FYComb/4+FXComb-FZComb/2)*PMO2;
						FMasaN2Reactivos=FMolesCombQuemado*(FYComb/4+FXComb-FZComb/2)*FRelacionMolarN2_O2*PMN2;
						...
						FMasaH2OProductos=FMolesCombQuemado*(FYComb/2/*+(FYComb/4+FXComb-FZComb/2)*FRelacionMolarH2O_O2*/)*PMH2O;
						FMasaN2Productos=FMolesCombQuemado*(FYComb/4+FXComb-FZComb/2)*FRelacionMolarN2_O2*PMN2;

						FFraccionMasicaEspecie[0]=(FMasaEspecie[0]-FMasaO2Reactivos-FFraccionMasicaEspecie[0]*FMasaBlowBy)/FMasa;  // Y O2
						FFraccionMasicaEspecie[1]=(FMasaEspecie[1]+FMasaCO2Productos-FFraccionMasicaEspecie[1]*FMasaBlowBy)/FMasa; // Y CO2
						FFraccionMasicaEspecie[2]=(FMasaEspecie[2]+FMasaH2OProductos-FFraccionMasicaEspecie[2]*FMasaBlowBy/*-FMasaH2OReactivos*/)/FMasa;  // Y H2O
						FFraccionMasicaEspecie[7]=(FMasaEspecie[7]-FFraccionMasicaEspecie[7]*FMasaBlowBy)/FMasa;    // Y N2
						FFraccionMasicaEspecie[3]=FComposicionCicloCerrado[1];        // Combustible que queda sin quemar
			...
			if(FHayEGR){
				FFraccionMasicaEspecie[FMotor->getSpeciesNumber()-1]=(FMasaEspecie[FMotor->getSpeciesNumber()-1]-FFraccionMasicaEspecie[FMotor->getSpeciesNumber()-1]*FMasaBlowBy)/FMasa;
		...
		if(FAnguloActual>FDistribucion.CA && FAnguloAnterior<=FDistribucion.CA){
			if(FMasaFuel==0.){
				...
				FAFR=FMasaEspecieCicloCerrado[2]/FMasaFuel;  // Masa Aire Fresco Inicial al cierre/Masa Fuel
			}
		}
		...
		while(!CotaError){
			...
			H1=0.;
			for(int i=0;i<FNumeroUnionesAdm;i++){
				if(dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getMassflow()!=0 && dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getSentidoFlujo()==nmEntrante){
					if(PrimerPaso){
						...
						-dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getMassflow()*FDeltaT,ASon1/ARef,FMasa0);
					}else{
						H1+=EntalpiaEntrada(dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getSpeedsound(),
						dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getVelocity(),
						-dynamic_cast<TCCCilindro *>(FCCValvulaAdm[i])->getMassflow()*FDeltaT,ASon1/ARef,FMasa);
					}
				}
			}
			//ENTALPIA POR ESCAPE;
			for(int i=0;i<FNumeroUnionesEsc;i++){
				if(dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getMassflow()!=0 && dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getSentidoFlujo()==nmEntrante){
					if(PrimerPaso){
						...
						-dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getMassflow()*FDeltaT,ASon1/ARef,FMasa0);
					}else{
						H1+=EntalpiaEntrada(dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getSpeedsound(),
						dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getVelocity(),
						-dynamic_cast<TCCCilindro *>(FCCValvulaEsc[i])->getMassflow()*FDeltaT,ASon1/ARef,FMasa);
					}
				}
			}
			if(PrimerPaso){
			...
			Energia=FVolumen0*FMasa/FVolumen/FMasa0*exp((H1+H0)/2.+(FCalor.TransTotal+FCalor.Liberado)*(MasTempMed/FRMezcla));
	...
	printf(" %lf %lf %lf \n ", FGamma,FRMezcla,FFraccionMasicaEspecie[0]);
		...
		FParInstantaneo=(FPressure-FMotor->getPresionAmb())*1e5*area*R*sin(a-b)/cos(b);
		...
		if(FAnguloActual<FAnguloAnterior){
			...
			FTrabajoNetoACUM=0.;
			FPMN=FTrabajoNeto*1e-5/FMotor->getGeometria().CilindradaUnitaria;
			FTrabajoBombeo=FTrabajoBombeoACUM;
			FTrabajoBombeoACUM=0.;
			FPMB=FTrabajoBombeo*1e-5/FMotor->getGeometria().CilindradaUnitaria;
			FPMI=FPMN-FPMB;
		}

		FDensidadReferencia=FMotor->getTuboRendVol()->GetGamma(FMotor->getNodoMedio())*FMotor->getTuboRendVol()->GetPresion(FMotor->getNodoMedio())*1e5/pow2(FMotor->getTuboRendVol()->GetAsonido(FMotor->getNodoMedio())*ARef);
		...
		if(FAnguloActual>FDistribucion.AA && FAnguloActual<FDistribucion.CE){
			if(MasaAdmInstante>0. && MasaEscInstante<0.){
				...
				MasaCortocircuitoEsc=MasaEscInstante*(Pi/2.-FAlphaEscape)/Pi;
				...
				for(int j=0;j<FMotor->getSpeciesNumber()-2;j++){
					...
					FComposicionSaliente[j]=FFraccionMasicaEspecie[j]*(MasaEscInstante+FMasaCortocircuito)/MasaEscInstante - FraccionCC*FMasaCortocircuito/MasaEscInstante;
					FraccionCC=0.;
					NumeroUnionesEntrante=0;
				}
				if(FHayEGR) FComposicionSaliente[FMotor->getSpeciesNumber()-1]=FFraccionMasicaEspecie[FMotor->getSpeciesNumber()-1];
			}else if(MasaAdmInstante<0. && MasaEscInstante>0.){
				// Cortocircuito con sentido Escape hacia Admisi�n.   (massflow negativo)
				MasaCortocircuitoAdm=MasaAdmInstante*(Pi/2.-FAlphaAdmision)/Pi;
				...
				for(int j=0;j<FMotor->getSpeciesNumber()-2;j++){
					...
					FComposicionSaliente[j]=FFraccionMasicaEspecie[j]*(MasaAdmInstante-FMasaCortocircuito)/MasaAdmInstante + FraccionCC*FMasaCortocircuito/MasaAdmInstante;
					FraccionCC=0.;
					NumeroUnionesEntrante=0;
				}
				if(FHayEGR) FComposicionSaliente[FMotor->getSpeciesNumber()-1]=FFraccionMasicaEspecie[FMotor->getSpeciesNumber()-1];
	...
	{
		std::cout << "ERROR: TCilindro2T::ActualizaPropiedades en cilindro: " << FNumeroCilindro << std::endl;
...
double TCilindro2T::EntalpiaEntrada(double ASonEnt,double VelEnt,double MasEnt,
double ASonCil,double MasCil)
{
	...
	{
		std::cout << "ERROR: TCilindro2T::EntalpiaEntrada en cilindro: " << FNumeroCilindro << std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
}
...
void TCilindro2T::VariableInicialesCicloACT()
{
	try
	{
		...
		Ftest_variables[1]=(FMasaPorAdmision-FMasaEGR)*1000;  //Measured air mass
		...
		Ftest_variables[11]=FTempPared[0].Culata+273;             //Cylinder head temperature
		Ftest_variables[12]=FTempPared[0].Cylinder+273;           //Cylinder temperature
		Ftest_variables[13]=FTempPared[0].Piston+273;             //Piston temperature
		if(FMotor->getSpeciesModel()==nmCalculoCompleto){
			Ftest_variables[14]=FFraccionMasicaEspecie[5]*1e6*1.587;   //NOx concentration at IVC
			Ftest_variables[15]=FFraccionMasicaEspecie[4]*1e6;         //SOOT concentration at IVC
			Ftest_variables[16]=FFraccionMasicaEspecie[6]*1e6;         //CO concentration at IVC
			Ftest_variables[17]=FFraccionMasicaEspecie[3]*1e6;         //HC concentration at IVC
		}
	}
	catch(Exception &N)
	{
		std::cout << "ERROR: TCilindro2T::VariableInicialesCicloACT en cilindro: " << FNumeroCilindro << std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
}
