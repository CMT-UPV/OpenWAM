			...
			if (FCalcComb == nmFQL) {
				...
				if (FMotor->getFTipoDatosIny() == 1) {
					...
					for (int i = 0; i < FNumIny; i++) {
						...
						//Si la inyección empieza en angulo negativo, hay que sumar 360º
...
//						}
					...
					if (FMotor->getLeyQuemadoBD()[0].Wiebes.size() == 4) {
						...
						//Si la combustión principal empieza en angulo negativo, hay que sumar 360º
						if (FMotor->getLeyQuemadoBD()[0].Wiebes[1].Alpha0 < 0) {
							...
							FAnguloInjeccion[0] = FMotor->getLeyQuemadoBD()[0].Wiebes[0].Alpha0
								+ FMotor->getAngTotalCiclo();
						} else {
							FAnguloInjeccion[1] = FMotor->getLeyQuemadoBD()[0].Wiebes[1].Alpha0;
							FAnguloInjeccion[0] = FMotor->getLeyQuemadoBD()[0].Wiebes[0].Alpha0
		...
		} else if (FMotor->getGammaCalculation() == nmComposicion
			|| FMotor->getGammaCalculation() == nmComposicionTemperatura) {
			if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
				if (FMotor->getSpeciesNumber() == 9) {
					FFraccionMasicaEspecieFuel = 0; // No se tiene en cuenta el combustible
				} else if (FMotor->getSpeciesNumber() == 10) {
		...
		if (!FCicloCerrado && FDeltaAngulo > 0.) {
			// GASTO POR ADMISION
			for (int i = 0; i < FNumeroUnionesAdm; i++) {
				FMasaValvAdm = -dynamic_cast<TCCCilindro*>(FCCValvulaAdm[i])->getMassflow() * FDeltaT; // signo - xq el massflow entrante se calcula negativo en la BC. Asi en el cilindro sera positivo si es entrante y viceversa.
				...
				if (FHayEGR)
					FAcumMasaEGR += FCCValvulaAdm[i]->GetFraccionMasicaEspecie(FMotor->getSpeciesNumber() - 1)
						* FMasaValvAdm;
			}
			// GASTO POR ESCAPE
			for (int i = 0; i < FNumeroUnionesEsc; i++) {
				masavalesc = -dynamic_cast<TCCCilindro*>(FCCValvulaEsc[i])->getMassflow() * FDeltaT; // signo - xq el massflow entrante se calcula negativo en la BC. Asi en el cilindro sera positivo si es entrante y viceversa.
		...
		if (FPressure > FMotor->getPresionAmb() && FMotor->getGeometria().CDBlowBy > 0.) {
			...
			z = FGamma4
				* (pow(FPresionCarter / FPressure, 2 / FGamma) - pow(FPresionCarter / FPressure, FGamma2 / FGamma));
			FGastoBlowBy = FMotor->getGeometria().CDBlowBy * 3.5e-5 * FMotor->getGeometria().Diametro
				* __units::BarToPa(FPressure) * sqrt(z / FRMezcla / __units::degCToK(FTemperature));
		} else {
			FGastoBlowBy = 0.;
		}
		...
		if (FMotor->getCombustible() == nmMEP || FMotor->getCombustible() == nmMEC) {
			...
			double tcil = __units::degCToK(FTemperature);
			double visc = 7.457e-6 + 4.1547e-8 * tcil - 7.4793e-12 * pow2(tcil);
			double dens = __units::BarToPa(FPressure) / FRMezcla / tcil;
		...
		FCalor.TransCilindro = Fh * (FTempPared[0].Cylinder - FTemperature) * 4
			* (FVolumen / 2. + FVolumen0 / 2. - FMotor->getGeometria().VCC) / FMotor->getGeometria().Diametro;
			...
			if (FAnguloComb < FIniComb || FAnguloComb > FFinComb || FMasaFuel == 0.) {
				...
				if (FMairequem > (FMasaEspecieCicloCerrado[2])) {
					...
					if (!FSaturado) {
						...
						FMasaEspecieCicloCerrado[0] = FMasaEspecieCicloCerrado[0] + FMairequem + mfquefin
					...
					FMasaEspecieCicloCerrado[0] = FMasaEspecieCicloCerrado[0] + FMairequem + FMfquem
						- FComposicionCicloCerrado[0] * FMasaBlowBy; // Gases Quemados
		...
		if (FCicloCerrado) {
			if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
				if (FMotor->getACT()) {
					...
					FFraccionMasicaEspecie[0] = (FSpecies_IVC[1] * (1 - FCalor.FQL)
						+ dataOUT.species_EVO[1] * FCalor.FQL) * (1 - Yfuel);
					FFraccionMasicaEspecie[1] = (FSpecies_IVC[2] * (1 - FCalor.FQL)
						+ dataOUT.species_EVO[2] * FCalor.FQL) * (1 - Yfuel);
					FFraccionMasicaEspecie[2] = (FSpecies_IVC[3] * (1 - FCalor.FQL)
						+ dataOUT.species_EVO[3] * FCalor.FQL) * (1 - Yfuel);
					FFraccionMasicaEspecie[3] = (FSpecies_IVC[7] * (1 - FCalor.FQL)
						+ dataOUT.species_EVO[7] * FCalor.FQL) * (1 - Yfuel);
					FFraccionMasicaEspecie[4] = (FSpecies_IVC[6] * (1 - FCalor.FQL)
						+ dataOUT.species_EVO[6] * FCalor.FQL) * (1 - Yfuel);
					FFraccionMasicaEspecie[5] = (FSpecies_IVC[4] * (1 - FCalor.FQL)
						+ dataOUT.species_EVO[4] * FCalor.FQL) * (1 - Yfuel);
					FFraccionMasicaEspecie[6] = (FSpecies_IVC[5] * (1 - FCalor.FQL)
					...
					if (FMotor->getSpeciesNumber() == 9) {
						...
						FMasaN2Reactivos = FMolesCombQuemado * (FYComb / 4 + FXComb - FZComb / 2) * FRelacionMolarN2_O2
							* __PM::N2;
						...
						FMasaN2Productos = FMolesCombQuemado * (FYComb / 4 + FXComb - FZComb / 2) * FRelacionMolarN2_O2
							* __PM::N2;

						FFraccionMasicaEspecie[0] = (FMasaEspecie[0] - FMasaO2Reactivos
							- FFraccionMasicaEspecie[0] * FMasaBlowBy) / FMasa;
						// Y O2
						FFraccionMasicaEspecie[1] = (FMasaEspecie[1] + FMasaCO2Productos
							- FFraccionMasicaEspecie[1] * FMasaBlowBy) / FMasa;
						// Y CO2
						FFraccionMasicaEspecie[2] = (FMasaEspecie[2] + FMasaH2OProductos
							- FFraccionMasicaEspecie[2] * FMasaBlowBy /* -FMasaH2OReactivos */) / FMasa;
						// Y H2O
						...
						FFraccionMasicaEspecie[2] = (FMasaEspecie[2] + FMasaH2OProductos
							- FFraccionMasicaEspecie[2] * FMasaBlowBy /* -FMasaH2OReactivos */) / FMasa;
						// Y H2O
						FFraccionMasicaEspecie[8] = (FMasaEspecie[8] - FFraccionMasicaEspecie[7] * FMasaBlowBy) / FMasa; // Y N2
			...
			FMasaEspecie[FMotor->getSpeciesNumber() - 2] = FFraccionMasicaEspecie[FMotor->getSpeciesNumber() - 2]
				* FMasa;
			if (FHayEGR) {
				...
				FMasaEspecie[FMotor->getSpeciesNumber() - 1] = FFraccionMasicaEspecie[FMotor->getSpeciesNumber() - 1]
					* FMasa;
			}
		}
...
//		}
		...
		while (!CotaError) {
			...
			Energia = FVolumen0 * FMasa / FVolumen / FMasa0
				* exp((H1 + H0) / 2. + (FCalor.TransTotal + FCalor.Liberado + FecgInt) * (MasTempMed / FRMezcla));
		...
		FDensidadReferencia = FMotor->getTuboRendVol()->GetGamma(FMotor->getNodoMedio())
			* __units::BarToPa(FMotor->getTuboRendVol()->GetPresion(FMotor->getNodoMedio()))
			/ pow2(FMotor->getTuboRendVol()->GetAsonido(FMotor->getNodoMedio()) * __cons::ARef);
		...
		if (FAnguloActual > FDistribucion.AA && FAnguloActual < FDistribucion.CE) {
			if (MasaAdmInstante > 0. && MasaEscInstante < 0.) {
				...
				for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
					...
					FComposicionSaliente[j] = FFraccionMasicaEspecie[j] * (MasaEscInstante + FMasaCortocircuito)
						/ MasaEscInstante - FraccionCC * FMasaCortocircuito / MasaEscInstante;
					FraccionCC = 0.;
					NumeroUnionesEntrante = 0;
				}
				if (FHayEGR)
					FComposicionSaliente[FMotor->getSpeciesNumber() - 1] = FFraccionMasicaEspecie[FMotor
						->getSpeciesNumber() - 1];
				...
				for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
					...
					FComposicionSaliente[j] = FFraccionMasicaEspecie[j] * (MasaAdmInstante - FMasaCortocircuito)
						/ MasaAdmInstante + FraccionCC * FMasaCortocircuito / MasaAdmInstante;
					FraccionCC = 0.;
					NumeroUnionesEntrante = 0;
				}
				if (FHayEGR)
...
double TCilindro2T::EntalpiaEntrada(double ASonEnt, double VelEnt, double MasEnt, double ASonCil, double MasCil) {
	try {
...
void TCilindro2T::VariableInicialesCicloACT() {
	try {
		...
		std::cout << "____________________________________________________" << std::endl;
		std::cout << "INFO: Engine speed:        " << Ftest_variables[0] << " (rpm)" << std::endl;
		std::cout << "INFO: Intake mass:         " << Ftest_variables[1] << " (kg/s)" << std::endl;
		std::cout << "INFO: Trapped mass:        " << Ftest_variables[2] << " (g)" << std::endl;
		std::cout << "INFO: Temperature at IVC:  " << Ftest_variables[3] << " (K)" << std::endl;
		std::cout << "INFO: Fuel mass:           " << Ftest_variables[4] << " (mg)" << std::endl;
		std::cout << "INFO: Injection pressure:  " << Ftest_variables[5] << " (bar)" << std::endl;
		std::cout << "INFO: Intake pressure:     " << Ftest_variables[6] << " (Pa)" << std::endl;
		std::cout << "INFO: Exhaust pressure:    " << Ftest_variables[7] << " (Pa)" << std::endl;
		std::cout << "____________________________________________________" << std::endl;
	} catch (exception & N) {
