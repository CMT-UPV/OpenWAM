		...
		if (FCicloCerrado && FPrimerInstanteCicloCerrado) {
			...
			if (FMotor->getACT()) {
				...
				for (int i = 0; i < FIN; i++) {
					...
					FMFI[i] = FMotor->getInjecPulse(i).Masa
							* FMotor->getMasaFuel() * 1e6;
				...
				ACT(Fengine_parameters, Fengine_model_constants,
						Ftest_variables, Finjection_rate, FCAD_injection_rate,
						Fsize_inlet_inj, FIN, FSOI, FEOI, FCAI, FCAD_exit,
						FHRF_exit, FROHR_exit, Fp_cyl_exit, Fdp_da_cyl_exit,
						FT_cyl_exit, FH_cooler_exit, Fmean_var_exit,
						Fheat_transfer, Finjection_rate_exit,
...
//						}
					...
					if (FMotor->getLeyQuemadoBD()[0].Wiebes.size() == 4) {
						...
						FTInyeccion[1] = (FFinComb - FIniComb)
								/ (FMotor->getRegimen() * 6.) / 4.;
						FPercentInyeccion[0] =
								FMotor->getLeyQuemadoBD()[0].Wiebes[0].Beta;
						...
						if (FMotor->getLeyQuemadoBD()[0].Wiebes[1].Alpha0 < 0) {
							FAnguloInjeccion[1] =
									FMotor->getLeyQuemadoBD()[0].Wiebes[1].Alpha0
											+ FMotor->getAngTotalCiclo();
							FAnguloInjeccion[0] =
									FMotor->getLeyQuemadoBD()[0].Wiebes[0].Alpha0
											+ FMotor->getAngTotalCiclo();
						} else {
							FAnguloInjeccion[1] =
									FMotor->getLeyQuemadoBD()[0].Wiebes[1].Alpha0;
						...
						FTInyeccion[0] = (FFinComb - FIniComb)
								/ (FMotor->getRegimen() * 6.) / 4.;
						FAnguloInjeccion[0] = FIniComb
								- FAnguloRetrasoCombustion
								+ FMotor->getAngTotalCiclo(); // Se asume que la inyección empieza cuando empieza la FQL
						FPercentInyeccion[0] = 1;
					}
				}
		...
		FDeltaAngulo = FMotor->getAngTotalCiclo() * FMotor->getRegimen() / 60.
				* FDeltaT;
		...
		if (FAnguloActual > 180.) { // Hector 2T
			FAnguloComb0 = FAnguloActual - FMotor->getAngTotalCiclo()
					- FDeltaAngulo; // Hector 2T
		...
		if (FMotor->getACT()) {
			for (int i = 0; i < FNumeroUnionesAdm; i++) {
				FPresionMedAdm +=
						FCCValvulaAdm[i]->GetTuboExtremo(0).Pipe->GetPresion(0)
								* FDeltaT / FNumeroUnionesAdm;
			}
			for (int i = 0; i < FNumeroUnionesEsc; i++) {
				FPresionMedEsc +=
						FCCValvulaEsc[i]->GetTuboExtremo(0).Pipe->GetPresion(0)
			...
			for (int i = 0; i < FNumeroUnionesEsc; i++) {
				...
				if (FAnguloActual - FDeltaAngulo < FDistribucion.CE
					...
					masavalesc = masavalesc
							* (FDistribucion.CE - FAnguloActual + FDeltaAngulo)
		...
		/* MASA DE BLOW-BY */
		/* ================================= */
		if (FPressure > FMotor->getPresionAmb()
				&& FMotor->getGeometria().CDBlowBy > 0.) {
			if (FMotor->getPresionAmb() / FPressure
					< pow(2. / FGamma2, FGamma6)) {
			...
			z =
					FGamma4
							* (pow(FPresionCarter / FPressure, 2 / FGamma)
		...
		for (int i = 0; i < FNumIny; i++) {
			if (FAnguloActual > FAnguloInjeccion[i]
					&& FAnguloAnterior <= FAnguloInjeccion[i]
		...
		if (FAnguloActual > FDistribucion.CA
				&& FAnguloAnterior <= FDistribucion.CA && !FCicloCerrado) {
			...
			std::cout << "____________________________________________________"
					<< std::endl;
			std::cout << "INFO: Intake mass:         " << FMasaPorAdmision * 1e3
					<< " (g)" << std::endl;
			std::cout << "INFO: Exhaust mass:        " << FMasaPorEscape * 1e3
					<< " (g)" << std::endl;
		...
		if (FMotor->getCombustible() == nmMEP
			...
			double condk = 6.1944e-3 + 7.3814e-5 * tcil
					- 1.2491e-8 * pow2(tcil);
			...
			if (FAnguloActual >= 0. && FAnguloActual <= 90.) {
				Fequis = Fratioctm
						+ 1
								/ (pow(cosh(FAnguloActual / 100.), 40.)
										+ Fratioctm / (1 - Fratioctm));
			} else if (FAnguloActual >= 270. && FAnguloActual <= 360.) {
				Fequis = Fratioctm
						+ 1
								/ (pow(cosh((360. - FAnguloActual) / 100.), 40.)
										+ Fratioctm / (1 - Fratioctm));
			} else {
				Fequis = 0.;
			}
			FCu = (2. * Pi * FMotor->getRegimen() / 60) * Fequis * Fctorbadmp
					* FKctm * pow2(FMotor->getGeometria().Diametro)
			...
			double deltaP = (FPressure
					- FPresionRCA * pow(FVolumenCA / FVolumen, 1.36)) * 1e5;
			if (deltaP < 0.)
				deltaP = 0.;
			if (FAnguloActual >= 90. && FAnguloActual <= 180.) {
				// CARRERA DE ESCAPE
				Fh =
						1.2e-2 * pow(FPressure * 1e5, 0.8)
								* pow(FTemperature + 273., -0.53)
								* pow(FMotor->getGeometria().Diametro, -0.2)
								* pow(
										(6.18 * FCm + 0.417 * FCu)
												* FMotor->getAjusteTranCalEsc()
												+ Fc2 * deltaP
		...
		FCalor.TransCilindro = Fh * (FTempPared[0].Cylinder - FTemperature) * 4
				* (FVolumen / 2. + FVolumen0 / 2. - FMotor->getGeometria().VCC)
		...
		if (FCicloCerrado) {
			if (FAnguloComb < FIniComb || FAnguloComb > FFinComb
				...
				FCalor.Liberado = (FCalor.FQL - FCalor.FQL0) * FMfint
						* FMotor->getPoderCalorifico()
				...
				if (FMairequem > (FMasaEspecieCicloCerrado[2])) {
					...
					if (!FSaturado) {
						...
						FCalor.Liberado = (FCalor.FQL - FCalor.FQL0) * mfquefin
								* FMotor->getPoderCalorifico()
								* FMotor->getRendimientoCombustion();
						...
						FMasaEspecieCicloCerrado[0] =
								FMasaEspecieCicloCerrado[0] + FMairequem
										+ mfquefin
										- FComposicionCicloCerrado[0]
												* FMasaBlowBy;
						// Gases Quemados
						FMasaEspecieCicloCerrado[1] =
								FMasaEspecieCicloCerrado[1] - mfquefin
										- FComposicionCicloCerrado[1]
					...
					FMasaEspecieCicloCerrado[1] = FMasaEspecieCicloCerrado[1]
							- FMfquem
							- FComposicionCicloCerrado[1] * FMasaBlowBy
							+ FFuelInstant; // Combustible
					FMasaEspecieCicloCerrado[2] = FMasaEspecieCicloCerrado[2]
							- FMairequem
							- FComposicionCicloCerrado[2] * FMasaBlowBy; // Aire Fresco
		...
		if (FCicloCerrado) {
			if (FMotor->getSpeciesModel() == nmCalculoCompleto) {
				if (FMotor->getACT()) {
					...
					FFraccionMasicaEspecie[0] = (FSpecies_IVC[1]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[1] * FCalor.FQL)
							* (1 - Yfuel);
					FFraccionMasicaEspecie[1] = (FSpecies_IVC[2]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[2] * FCalor.FQL)
							* (1 - Yfuel);
					FFraccionMasicaEspecie[2] = (FSpecies_IVC[3]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[3] * FCalor.FQL)
							* (1 - Yfuel);
					FFraccionMasicaEspecie[3] = (FSpecies_IVC[7]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[7] * FCalor.FQL)
							* (1 - Yfuel);
					FFraccionMasicaEspecie[4] = (FSpecies_IVC[6]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[6] * FCalor.FQL)
							* (1 - Yfuel);
					FFraccionMasicaEspecie[5] = (FSpecies_IVC[4]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[4] * FCalor.FQL)
							* (1 - Yfuel);
					FFraccionMasicaEspecie[6] = (FSpecies_IVC[5]
							* (1 - FCalor.FQL)
							+ dataOUT.species_EVO[5] * FCalor.FQL)
							* (1 - Yfuel);

					if (FMotor->getSpeciesNumber() == 9) {
						FFraccionMasicaEspecie[3] += Yfuel;
						FFraccionMasicaEspecie[7] = (FSpecies_IVC[0]
								* (1 - FCalor.FQL)
						...
						FFraccionMasicaEspecie[8] = (FSpecies_IVC[0]
								* (1 - FCalor.FQL)
								+ dataOUT.species_EVO[0] * FCalor.FQL)
								* (1 - Yfuel);
					}
					...
					if (FMotor->getSpeciesNumber() == 9) {
						if (!FSaturado) {
							FMolesCombQuemado = FMfquem
									/ (FXComb * 12.01 + FYComb * 1.01
											+ FZComb * 16); // Moles de combustible quemado en el incremento temporal calculado
						} else {
						...
						FFraccionMasicaEspecie[0] = (FMasaEspecie[0]
								- FMasaO2Reactivos
								- FFraccionMasicaEspecie[0] * FMasaBlowBy)
								/ FMasa;
						// Y O2
						FFraccionMasicaEspecie[1] = (FMasaEspecie[1]
								+ FMasaCO2Productos
								- FFraccionMasicaEspecie[1] * FMasaBlowBy)
								/ FMasa;
						// Y CO2
						FFraccionMasicaEspecie[2] =
								(FMasaEspecie[2] + FMasaH2OProductos
										- FFraccionMasicaEspecie[2]
												* FMasaBlowBy /* -FMasaH2OReactivos */)
										/ FMasa;
						// Y H2O
						FFraccionMasicaEspecie[7] = (FMasaEspecie[7]
								- FFraccionMasicaEspecie[7] * FMasaBlowBy)
								/ FMasa; // Y N2
						...
						for (int j = 0;
								j < FMotor->getSpeciesNumber() - FIntEGR; j++) {
							FMasaEspecie[j] = FFraccionMasicaEspecie[j] * FMasa;
						}
						...
						FFraccionMasicaEspecie[2] =
								(FMasaEspecie[2] + FMasaH2OProductos
										...
										/ FMasa;
						// Y H2O
						FFraccionMasicaEspecie[8] = (FMasaEspecie[8]
								- FFraccionMasicaEspecie[7] * FMasaBlowBy)
								/ FMasa; // Y N2
			...
			for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
				FFraccionMasicaEspecie[j] = (FMasaEspecie[j]
						- FFraccionMasicaEspecie[j] * FMasaBlowBy) / FMasa;
				FraccionMasicaAcum += FFraccionMasicaEspecie[j];
				FMasaEspecie[j] = FFraccionMasicaEspecie[j] * FMasa;
			}
			FFraccionMasicaEspecie[FMotor->getSpeciesNumber() - 2] = 1.
					- FraccionMasicaAcum;
		...
		if (FAnguloActual > FDistribucion.CA
				&& FAnguloAnterior <= FDistribucion.CA) {
			if (FMasaFuel == 0.) {
				FAFR = 0.;
			} else {
...
//		}
		...
		while (!CotaError) {
			...
			for (int i = 0; i < FNumeroUnionesAdm; i++) {
				if (dynamic_cast<TCCCilindro*>(FCCValvulaAdm[i])->getMassflow()
						!= 0
						&& dynamic_cast<TCCCilindro*>(FCCValvulaAdm[i])->getSentidoFlujo()
								== nmEntrante) {
					if (PrimerPaso) {
						H1 +=
								EntalpiaEntrada(
			...
			for (int i = 0; i < FNumeroUnionesEsc; i++) {
				if (dynamic_cast<TCCCilindro*>(FCCValvulaEsc[i])->getMassflow()
						!= 0
						&& dynamic_cast<TCCCilindro*>(FCCValvulaEsc[i])->getSentidoFlujo()
								== nmEntrante) {
					if (PrimerPaso) {
						H1 +=
								EntalpiaEntrada(
		...
		if (FAnguloActual > FDistribucion.AE
			...
			std::cout << "____________________________________________________"
					<< std::endl << std::endl;
			std::cout << "INFO: Calculated pressure at E.O.:    " << FPressure
					<< " (bar)" << std::endl;
		...
		FParInstantaneo = (FPressure - FMotor->getPresionAmb()) * 1e5 * area * R
				* sin(a - b) / cos(b);
		...
		FDensidadReferencia = FMotor->getTuboRendVol()->GetGamma(
				FMotor->getNodoMedio())
				* FMotor->getTuboRendVol()->GetPresion(FMotor->getNodoMedio())
				* 1e5
				/ pow2(
						FMotor->getTuboRendVol()->GetAsonido(
								FMotor->getNodoMedio()) * ARef);

		/* ================================= */
		/* MODELO DE CORTOCIRCUITO */
		/* ================================= */

		if (FAnguloActual > FDistribucion.AA
				&& FAnguloActual < FDistribucion.CE) {
				...
				for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
					for (int i = 0; i < FNumeroUnionesAdm; i++) {
						if (dynamic_cast<TCCCilindro*>(FCCValvulaAdm[i])->getSentidoFlujo()
								== nmEntrante) {
							FraccionCC +=
									FCCValvulaAdm[i]->GetFraccionMasicaEspecie(
											j);
				...
				for (int j = 0; j < FMotor->getSpeciesNumber() - 2; j++) {
					for (int i = 0; i < FNumeroUnionesEsc; i++) {
						if (dynamic_cast<TCCCilindro*>(FCCValvulaEsc[i])->getSentidoFlujo()
								== nmEntrante) {
							FraccionCC +=
									FCCValvulaEsc[i]->GetFraccionMasicaEspecie(
											j);
							NumeroUnionesEntrante++;
						}
					}
