...
void TBloqueMotor::LeeMotor(const char *FileWAM, fpos_t &filepos,
	...
	try {
		...
		FGeom.CilindradaUnitaria = Pi * pow2(FGeom.Diametro) * FGeom.Carrera
				/ 4.;
...
void TBloqueMotor::LeeMotorXML(xml_node node_openwam,
		nmTipoModelado& SimulationType, int CiclosSinInerciaTermica,
		nmTipoMotor EngineType, double *AtmosphericComposition) {
	try {
		...
		if (FACT) {
			...
			if (!FHayEGR) {
				std::cout
						...
						<< std::endl;
				std::cout
						<< "         has EGR, you must select the opiton 'To calculate"
						<< std::endl;
				std::cout
						<< "         EGR transport, in the other case, the results"
						<< std::endl;
		...
		FImponerComposicionAE = GetAttributeAsBool(node_enginitial,
				"ImposeComposition");
		...
		ImposeCompositionXML(nod_compini, FComposicionInicial, FHayEGR,
				FHayFuel, FCalculoEspecies);
		...
		if (FCombustible == nmMEC) {
			...
			FDosadoInicial = GetAttributeAsDouble(node_enginitial,
					"FuelToAirRatio");
		}
		...
		for (xml_node node_wall = node_heattr.child("Htr:Wall"); node_wall;
			...
			if (node_wall.attribute("Type").value() == "Piston") {
				...
				FParedPiston.Espesor = GetAttributeAsDouble(node_wall,
						"Thickness");
				FParedPiston.Conductividad = GetAttributeAsDouble(node_wall,
						"Conductivity");
				FParedPiston.Density = GetAttributeAsDouble(node_wall,
						"Density");
				FParedPiston.CalorEspecifico = GetAttributeAsDouble(node_wall,
						"HeatCapacity");
			} else if (node_wall.attribute("Type").value() == "Cylinder") {
				FTempInicial.Piston = GetAttributeAsDouble(node_wall,
						"Temperature");
				FParedCilindro.Espesor = GetAttributeAsDouble(node_wall,
						"Thickness");
				FParedCilindro.Conductividad = GetAttributeAsDouble(node_wall,
						"Conductivity");
				FParedCilindro.Density = GetAttributeAsDouble(node_wall,
						"Density");
				FParedCilindro.CalorEspecifico = GetAttributeAsDouble(node_wall,
						"HeatCapacity");
			} else if (node_wall.attribute("Type").value() == "CylHead") {
				FTempInicial.Piston = GetAttributeAsDouble(node_wall,
						"Temperature");
				FGeom.AreaPiston = GetAttributeAsDouble(node_wall, "Area");
				FParedCulata.Espesor = GetAttributeAsDouble(node_wall,
						"Thickness");
				FParedCulata.Conductividad = GetAttributeAsDouble(node_wall,
						"Conductivity");
				FParedCulata.Density = GetAttributeAsDouble(node_wall,
						"Density");
				FParedCulata.CalorEspecifico = GetAttributeAsDouble(node_wall,
						"HeatCapacity");
			}
		...
		const char_t* TypeHT = node_heattr.attribute("Type").value();
		if (TypeHT == "Constant") {
			FCalculoPared = nmTempFija;
		...
		FGeom.DistanciaValvulas = GetAttributeAsDouble(node_geom,
				"ValvesSeparation");
		...
		FGeom.DiametroBulon = GetAttributeAsDouble(node_geom,
				"PistonPinDiameter");
		FGeom.AlturaCoronaPiston = GetAttributeAsDouble(node_geom,
				"PistonHEight");
		FGeom.MasaBiela = GetAttributeAsDouble(node_geom, "MassConnectingRod");
		FGeom.MasaPistonSegmentosBulon = GetAttributeAsDouble(node_geom,
				"MassPistonPinRing");
		FGeom.ModuloElasticidad = GetAttributeAsDouble(node_geom,
				"ElasticityCoef");
		FGeom.CoefDeformaciones = GetAttributeAsDouble(node_geom,
				"DeformationCoef");

		FGeom.VCC = (Pi * FGeom.Diametro * FGeom.Diametro / 4. * FGeom.Carrera)
				/ (FGeom.RelaCompresion - 1.);
		FGeom.CilindradaUnitaria = Pi * pow(FGeom.Diametro, 2.) * FGeom.Carrera
				/ 4.;
		FGeom.CilindradaTotal = FGeom.CilindradaUnitaria
				* (double) FGeom.NCilin;
		...
		if (SimulationType == nmTransitorioRegimen) {
			...
			FRelCajaCambios = GetAttributeAsDouble(node_vehicle,
					"GearBoxRatio");
			FRendCajaCambios = GetAttributeAsDouble(node_vehicle,
					"GearBoxEfficiency");
			FRelTrasmision = GetAttributeAsDouble(node_vehicle,
					"TransmissionRatio");
			...
			FInerciaTotal = pow(FRelCajaCambios * FRelTrasmision, 2.) * Imc
					+ pow(FRelTrasmision, 2.) * Ict + Itr
					+ FMasaTotalVehiculo * pow(FRadioRueda, 2.);

			FPendiente = FMasaTotalVehiculo * 9.81 * sin(FAnguloCarretera);

			FCoeficienteInercias =
					FRendCajaCambios * pow(FRelCajaCambios * FRelTrasmision, 2.)
							/ (FInerciaTotal
									+ FMasaTotalVehiculo * pow(FRadioRueda, 2.));

			// Read the coeficients to obtain Road Load
			FCoefRoadLoad.A0 = GetAttributeAsDouble(node_vehicle,
					"RoadLoad_A0");
			FCoefRoadLoad.B0 = GetAttributeAsDouble(node_vehicle,
					"RoadLoad_B0");
			FCoefRoadLoad.C0 = GetAttributeAsDouble(node_vehicle,
					"RoadLoad_C0");
			FCoefRoadLoad.n = GetAttributeAsDouble(node_vehicle, "RoadLoad_n");
			FCoefRoadLoad.cd = GetAttributeAsDouble(node_vehicle,
					"RoadLoad_cd");
			FCoefRoadLoad.rho = GetAttributeAsDouble(node_vehicle,
					"RoadLoad_rho");
		...
		if (FACT) {
			...
			FInjectionSys.HoleNumber = GetAttributeAsInt(node_injsys,
					"HoleNumber");
			FInjectionSys.HoleDiame = GetAttributeAsDouble(node_injsys,
					"HoleDiameter");
			FInjectionSys.CDHole = GetAttributeAsDouble(node_injsys, "CDHole");
			FInjectionSys.InjectPressure = GetAttributeAsDouble(node_injsys,
					"InjectPressure");

			FInjectionSys.PendOpen_A1 = GetAttributeAsDouble(node_injsys,
					"SlopeOpen_A1");
			FInjectionSys.PendOpen_A2 = GetAttributeAsDouble(node_injsys,
					"SlopeOpen_A2");
			FInjectionSys.LevMax_B1 = GetAttributeAsDouble(node_injsys,
					"MaxOpen_B1");
			FInjectionSys.LevMax_B2 = GetAttributeAsDouble(node_injsys,
					"MaxOpen_B2");
			FInjectionSys.PendClose_C1 = GetAttributeAsDouble(node_injsys,
					"SlopeClose_C1");
			FInjectionSys.PendClose_C2 = GetAttributeAsDouble(node_injsys,
					"SlopeClose_C2");
			FInjectionSys.Desfase_D1 = GetAttributeAsDouble(node_injsys,
					"Shift_D1");
			FInjectionSys.Desfase_D2 = GetAttributeAsDouble(node_injsys,
					"Shift_D2");
			...
			for (xml_node node_hrl = node_hrldb.child("Hdb:HRL"); node_hrl;
				...
				for (xml_node node_wiebe = node_hrl.child("Hrl:Wiebe");
					...
					WiebeSimple.IncAlpha = GetAttributeAsDouble(node_wiebe,
							"IA");
		...
		if (FGeom.NCilin > 1) {
			const char_t* TypePh =
					node_engine.attribute("PhaseDifType").value();
			if (TypePh == "UserDef") {
				FTipoDesfase = nmPersonalizado;
			} else {
				FTipoDesfase = nmImpuesto;
		...
		for (xml_node node_ctrl = node_engine.child("Actuator"); node_ctrl;
			...
			if (CtrlParam == "RPM") {
				...
				FInjectionSys.InjectPCtrID = GetAttributeAsInt(node_ctrl,
						"CtrlID");
				FInjectionSys.InjectPCtrd = true;
		...
		for (xml_node node_cyl = node_engine.child("Eng:Cylinder"); node_cyl;
			...
			for (xml_node node_ctrl = node_cyl.child("Actuator"); node_ctrl;
				...
				const char_t* CtrlParam =
						node_ctrl.attribute("Parameter").value();
				if (CtrlParam == "Fuel") {
...
void TBloqueMotor::ResultadosMediosBloqueMotor() {
	try {
		...
		if (FResMediosMotor.Potencia) {
			FResMediosMotor.PotenciaMED = FResMediosMotor.ParEfectivoMED * 2.
					* Pi * FResMediosMotor.RegimenGiroMED / (60. * 1000);
		}
		if (FResMediosMotor.MasaAdmision) {
			for (int i = 0; i < FGeom.NCilin; i++) {
		...
		if (FResMediosMotor.RendimientoVolumetricoAtm) {
			DensidadAtm = FPresionAmbiente * 1e5
					/ (RAir * FTemperaturaAmbiente);
		...
		FResMediosMotor.PMNCicloMED = FResMediosMotor.TrabajoNetoMED * 1e-5
				/ FGeom.CilindradaTotal;
		...
		FResMediosMotor.PotenciaCicloMED = 2 * Pi
		...
		if (FResMediosMotor.MasaFuelMED == 0) {
			...
			FResMediosMotor.RendIndicadoMED = (FResMediosMotor.TrabajoNetoMED
					...
					/ (FGeom.NCilin * FResMediosMotor.MasaFuelMED
							* FPoderCalorifico);
			FResMediosMotor.RendEfectivoMED = FResMediosMotor.PMECicloMED
					* FGeom.CilindradaTotal * 1e5
					/ (FGeom.NCilin * FResMediosMotor.MasaFuelMED
							* FPoderCalorifico);
	...
	try {
		...
		if (FResMediosMotor.RendimientoVolumetrico || FResMediosMotor.Dosado) {
			FResMediosMotor.DensidadReferenciaSUM += FTuboRendVol->GetDensidad(
					FNodoMedio) * DeltaT;
			FResMediosMotor.GastoTuboReferenciaSUM +=
					(FTuboRendVol->GetVelocidad(FNodoMedio) * ARef
		...
		FResMediosMotor.TrabajoNetoSUM +=
				FCilindro[CilindroActual - 1]->getPreMed() * 1e5
	...
	try {
		...
		if (FRPMControlled) {
			...
			if (FTipoModelado == nmTransitorioRegimen) {
				...
				if (FTheta < 7200) {
					FRegimen = ((FParMotor - FParResistente)
							* FCoeficienteInercias * DeltaT + W) * 60
							/ (2 * Pi);
				}
