 ...
 Tfql::~Tfql()
{
  ...
  if(Fcombustion!=NULL) delete[] Fcombustion;
...
try
{
  ...
  */
  ...
  if(x>Fang0+719.75 && x<Fang0+720.25) Fcombustion[i]=true;
...
for (j=0;j<Fnley;++j){
     ...
     dist = pow( b,e);
     ley= fql(x,j,i);
     if(ley > 1.) printf("WARNING: Ley n. %d > 1. en %lf\n",j,x);
     up += ley / dist;
     down += 1./ dist;
