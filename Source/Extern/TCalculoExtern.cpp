...
if(FNumeroInMat>0) FEntrada=new double[FNumeroInMat];

if(FNumeroOutMat>0) FSalida=new double[FNumeroOutMat];
...
//  if(ECU!=NULL) delete ECU;
  ...
  if(FFuel!=NULL) delete FFuel;

  if(FEGRV!=NULL) delete FEGRV;

  if(FRegimen!=NULL) delete FRegimen;

  if(Ffql!=NULL) delete Ffql;

  if(FAjusteTransCalorCil!=NULL) delete FAjusteTransCalorCil;

  if(FFraccionMasicaCilindro!=NULL) delete FFraccionMasicaCilindro;

  if(FControlInyeccion!=NULL) delete FControlInyeccion;

  if(FInputs!=NULL) delete[] FInputs;

  if(FOutputs!=NULL) delete[] FOutputs;

  if(FEntrada!=NULL) delete[] FEntrada;

  if(FSalida!=NULL) delete[] FSalida;

  if(FSensorTubo!=NULL) delete[] FSensorTubo;

  if(FSensorDep!=NULL) delete[] FSensorDep;

  if(FSensorTG!=NULL) delete[] FSensorTG;

  if(FSensorTurbina!=NULL) delete[] FSensorTurbina;

  if(FSensorCil!=NULL) delete[] FSensorCil;

  if(FSensorVent!=NULL) delete[] FSensorVent;

  if(FSensorMotor!=NULL) delete[] FSensorMotor;
  ...
  if(FRutaTrabajo!=NULL) delete[] FRutaTrabajo;

  if(FParametros!=NULL) delete[] FParametros;
}
...
FOutputs[14]=FAjusteTransCalorCil->CEscape;*/
     ...
     FOutputs[2]=0;
     if(FOutputs[0]>1) FOutputs[0]=1;
     if(FOutputs[1]>1) FOutputs[1]=1;
   }
   catch(Exception &N)
   {
     ...
     if (FNSensTubos!=0){
                for (int i = 0; i <= ndistancias-1; ++i) {
                   ...
                   for (int j=0;j<=nsensortubo-1;++j) {
                     ...
                     fscanf(fich,"%d ",&tipo);
                     switch(tipo){
                     case 0: FSensorTubo[cont].tipo=nmPrTubo;
                             break;
                     case 1: FSensorTubo[cont].tipo=nmVelTubo;
                             break;
                     case 2: FSensorTubo[cont].tipo=nmTempTubo;
                             break;
                     case 3: FSensorTubo[cont].tipo=nmGastoTubo;
                             break;
                     case 4: FSensorTubo[cont].tipo=nmN2Tubo;
                             break;
                     case 5: FSensorTubo[cont].tipo=nmO2Tubo;
                             break;
                     case 6: FSensorTubo[cont].tipo=nmCO2Tubo;
                             break;
                     case 7: FSensorTubo[cont].tipo=nmH2OTubo;
                             break;
                     case 8: FSensorTubo[cont].tipo=nmHCTubo;
                             break;
                     case 9: FSensorTubo[cont].tipo=nmSootTubo;
                             break;
                     case 10: FSensorTubo[cont].tipo=nmNOxTubo;
                             break;
                     case 11: FSensorTubo[cont].tipo=nmCOTubo;
                             break;
                     case 12: FSensorTubo[cont].tipo=nmAireFrTubo;
                             break;
                     case 13: FSensorTubo[cont].tipo=nmGasQuemadoTubo;
                             break;
                     case 14: FSensorTubo[cont].tipo=nmEGRTubo;
                             break;
                     case 15: FSensorTubo[cont].tipo=nmCombustibleTubo;
                             break;
                     default: printf ("Error en el tipo de sensor\n");
                     throw Exception("");
                     } /*fin del switch*/
                     ...
                     case nmTempTubo: temp=Pipe[t]->GetAsonido(0)*ARef;
                             FSensorTubo[cont].valreal=temp*temp/Pipe[t]->GetGamma(0)/Pipe[t]->GetRMezcla(0)-273.;  // Como es el instante inicial da igual el nodo a que se hace referencia
                             ...
                             break;
                     case nmPrTubo: FSensorTubo[cont].valreal=Pipe[t]->getPresionInicial();
                             FSensorTubo[cont].valact=FSensorTubo[cont].valreal;
                             FSensorTubo[cont].valant=FSensorTubo[cont].valreal;
                             FSensorTubo[cont].valrealant=FSensorTubo[cont].valreal;
                             break;
                     case nmVelTubo: FSensorTubo[cont].valreal=Pipe[t]->getVelocidadMedia();
                             FSensorTubo[cont].valact=FSensorTubo[cont].valreal;
                             FSensorTubo[cont].valant=FSensorTubo[cont].valreal;
                             FSensorTubo[cont].valrealant=FSensorTubo[cont].valreal;
                             break;
                    case nmGastoTubo: temp=Pipe[t]->GetAsonido(0)*ARef; // Como es el instante inicial el nodo es arbitrario
     ...
     if (FNSensDepositos!=0){
                for (int i = 0; i <= ndep-1; ++i) {
                   ...
                   for (int j=0;j<=nsensdeposito-1;++j){
                     ...
                     fscanf(fich,"%d ",&tipo);
                     switch(tipo){
                     case 0: FSensorDep[cont].tipo=nmPrDep;
                             break;
                     case 1: FSensorDep[cont].tipo=nmTempDep;
                             break;
                     case 2: FSensorDep[cont].tipo=nmN2Dep;
                             break;
                     case 3: FSensorDep[cont].tipo=nmO2Dep;
                             break;
                     case 4: FSensorDep[cont].tipo=nmCO2Dep;
                             break;
                     case 5: FSensorDep[cont].tipo=nmH2ODep;
                             break;
                     case 6: FSensorDep[cont].tipo=nmHCDep;
                             break;
                     case 7: FSensorDep[cont].tipo=nmSootDep;
                             break;
                     case 8: FSensorDep[cont].tipo=nmNOxDep;
                             break;
                     case 9: FSensorDep[cont].tipo=nmCODep;
                             break;
                     case 10: FSensorDep[cont].tipo=nmAireFrDep;
                             break;
                     case 11: FSensorDep[cont].tipo=nmGasQuemadoDep;
                             break;
                     case 12: FSensorDep[cont].tipo=nmEGRDep;
                             break;
                     case 13: FSensorDep[cont].tipo=nmCombustibleDep;
                             break;
                     default: printf ("Error en el tipo de sensor\n");
                     throw Exception("");
                     } /*fin del switch*/
                     ...
                     case nmPrDep: FSensorDep[cont].valreal=Plenum[FSensorDep[cont].deposito-1]->getPressure();
                             ...
                             break;
                     case nmTempDep: FSensorDep[cont].valreal=Plenum[FSensorDep[cont].deposito-1]->getTemperature();
                             FSensorDep[cont].valact=FSensorDep[cont].valreal;
                             FSensorDep[cont].valant=FSensorDep[cont].valreal;
                             FSensorDep[cont].valrealant=FSensorDep[cont].valreal;
                             break;
     ...
     if (FNSensTurbogrupo!=0){
                for (int i = 0; i <= ntg-1; ++i) {
                   ...
                   for (int j=0;j<=nsensturbogrupo-1;++j){
                     ...
                     fscanf(fich,"%d ",&tipo);
                     switch(tipo){
                     case 0: FSensorTG[cont].tipo=nmRTCTG;
                             break;
                     default: printf ("Error en el tipo de sensor\n");
                     throw Exception("");
                     } /*fin del switch*/
                     ...
                     case nmRTCTG:   FSensorTG[cont].valreal=Axis[FSensorTG[cont].turbogrupo-1]->getRegimen();
     ...
     if (FNSensTurbina!=0){
                for (int i = 0; i <= nturbinas-1; ++i) {
                   ...
                   for (int j=0;j<=nsensturbina-1;++j){
                     ...
                     fscanf(fich,"%d ",&tipo);
                     switch(tipo){
                     case 0: FSensorTurbina[cont].tipo=nmRelaci1;
                             break;
                     case 1: FSensorTurbina[cont].tipo=nmRelaci2;
                             break;
                     default: printf ("Error en el tipo de sensor\n");
                     throw Exception("");
                     } /*fin del switch*/
                     ...
                     case nmRelaci1: FSensorTurbina[cont].valreal=Turbine[FSensorTurbina[cont].turbina-1]->GetRelacionCinematica(0);
                                     ...
                                     break;
                     case nmRelaci2: FSensorTurbina[cont].valreal=Turbine[FSensorTurbina[cont].turbina-1]->GetRelacionCinematica(1);
                                     FSensorTurbina[cont].valact=FSensorTurbina[cont].valreal;
                                     FSensorTurbina[cont].valant=FSensorTurbina[cont].valreal;
                                     FSensorTurbina[cont].valrealant=FSensorTurbina[cont].valreal;
                                     break;
                ...
                for (int i = 0; i <= ncil-1; ++i) {
      		   fscanf(fich,"%d %d ",&cilindro,&nsenscilindro);
                   for (int j=0;j<=nsenscilindro-1;++j){
                     ...
                     fscanf(fich,"%d ",&tipo);
                     switch(tipo){
                          case 0: FSensorCil[cont].tipo=nmPrCil;
                                  break;
                          case 1: FSensorCil[cont].tipo=nmTempCil;
                                  break;
                          case 2: FSensorCil[cont].tipo=nmMfCil;
                                  break;
                          case 3: FSensorCil[cont].tipo=nmAFRCil;
                                  break;
                          default: printf ("Error en el tipo de sensor\n");
                          throw Exception("");
                     } /*fin del switch*/
                     ...
                     case nmTempCil: FSensorCil[cont].valreal=Engine[0]->GetCilindro(FSensorCil[cont].cilindro-1)->getTemperature();
                                     ...
                                     break;
                     case nmPrCil: FSensorCil[cont].valreal=Engine[0]->GetCilindro(FSensorCil[cont].cilindro-1)->getPressure();
                                   FSensorCil[cont].valact=FSensorCil[cont].valreal;
                                   FSensorCil[cont].valant=FSensorCil[cont].valreal;
                                   FSensorCil[cont].valrealant=FSensorCil[cont].valreal;
                                   break;
					case nmMfCil  : FSensorCil[cont].valreal=Engine[0]->GetCilindro(FSensorCil[cont].cilindro-1)->getMasaFuel();
								   FSensorCil[cont].valact=FSensorCil[cont].valreal;
                                   FSensorCil[cont].valant=FSensorCil[cont].valreal;
                                   FSensorCil[cont].valrealant=FSensorCil[cont].valreal;
                                   break;
                     case nmAFRCil:FSensorCil[cont].valreal=15;
                                   FSensorCil[cont].valact=FSensorCil[cont].valreal;
                                   FSensorCil[cont].valant=FSensorCil[cont].valreal;
                                   FSensorCil[cont].valrealant=FSensorCil[cont].valreal;
                                   break;
     ...
     if (FNSensVenturis!=0){
                for (int i = 0; i <= nventuris-1; ++i) {
                   ...
                   for (int j=0;j<=nsensventuri-1;++j){
                     ...
                     fscanf(fich,"%d ",&tipo);
                     switch(tipo){
                          case 0: FSensorVent[cont].tipo=nmPrEntVent;
                                  break;
                          case 1: FSensorVent[cont].tipo=nmPrGarVent;
                                  break;
                          case 2: FSensorVent[cont].tipo=nmVelEntVent;
                                  break;
                          case 3: FSensorVent[cont].tipo=nmVelLatVent;
                                  break;
                          case 4: FSensorVent[cont].tipo=nmGastoEntVent;
                                  break;
                          case 5: FSensorVent[cont].tipo=nmGastoLatVent;
                                  break;
                          default: printf ("Error en el tipo de sensor\n");
                          throw Exception("");
                     } /*fin del switch*/
                     ...
                     case nmPrEntVent: prent=Venturi[v]->getPressure();
                                       ...
                                       break;
                     case nmPrGarVent: prent=Venturi[v]->getPressure();
prgar=prent/pow(1+Venturi[v]->getgamma3()*pow(dynamic_cast<TCCDeposito *>(Venturi[v]->getCCLateral())->getValvula()->getCRecuperacion(),2.),Venturi[v]->getGamma()/Venturi[v]->getgamma1());
									   FSensorVent[cont].valreal=prgar;
                                       FSensorVent[cont].valact=FSensorVent[cont].valreal;
                                       FSensorVent[cont].valant=FSensorVent[cont].valreal;
                                       FSensorVent[cont].valrealant=FSensorVent[cont].valreal;
                                       break;
					 case nmVelEntVent: if(dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())->getSentidoFlujo()==nmEntrante) {
										...
										velent=dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())->getVelocity()*signo*ARef;
										FSensorVent[cont].valreal=velent;
										FSensorVent[cont].valact=FSensorVent[cont].valreal;
										FSensorVent[cont].valant=FSensorVent[cont].valreal;
										FSensorVent[cont].valrealant=FSensorVent[cont].valreal;
										break;
					 case nmVelLatVent: vellat=dynamic_cast<TCCDeposito *>(Venturi[v]->getCCLateral())->getVelocity()*ARef;
                     ...
                     case nmGastoEntVent: gasent=-dynamic_cast<TCCDeposito *>(Venturi[v]->getCCEntrada())->getMassflow();  /*Massflow entrante positivo.Saliente negativo. Pedro */
                                          ...
                                          FSensorVent[cont].valrealant=FSensorVent[cont].valreal;
                                          break;
                     case nmGastoLatVent: gaslat=dynamic_cast<TCCDeposito *>(Venturi[v]->getCCLateral())->getMassflow();
     ...
     if (FNSensMotor!=0){
                for(int i=0;i<=FNSensMotor-1;++i) {
      		    fscanf(fich,"%d ",&tipo);
                    ...
                    switch(tipo){
                        case 0: FSensorMotor[i].tipo=nmAngulo;
                                break;
                        case 1: FSensorMotor[i].tipo=nmReg;
                                break;
                        case 2: FSensorMotor[i].tipo=nmTiempo;
                                break;
                        default: printf ("Error en el tipo de sensor\n");
                        throw Exception("");
                    } /*fin del switch*/
                    ...
                    case nmAngulo: FSensorMotor[i].valreal=CrankAngle;
                                   ...
                                   break;
                    case nmReg: FSensorMotor[i].valreal=ene;
                                FSensorMotor[i].valact=FSensorMotor[i].valreal;
                                FSensorMotor[i].valant=FSensorMotor[i].valreal;
                                FSensorMotor[i].valrealant=FSensorMotor[i].valreal;
                                break;
                    case nmTiempo: FSensorMotor[i].valreal=AcumulatedTime;
                                   FSensorMotor[i].valact=FSensorMotor[i].valreal;
                                   FSensorMotor[i].valant=FSensorMotor[i].valreal;
                                   FSensorMotor[i].valrealant=FSensorMotor[i].valreal;
                                   break;
     ...
     if(FNSensUED!=0){
         for(int i=0;i<=nUED-1; ++i) {
             ...
             for(int j=0;j<=nsensUED-1;++j){
                 ...
                 fscanf(fich,"%d ",&tipo);
                 switch(tipo){
                 case 0: FSensorUED[cont].tipo=nmGasto;
                         break;
                 default: printf ("Error en el tipo de sensor\n");
                 throw Exception("");
                 } /*fin del switch*/
                 ...
                 case nmGasto: FSensorUED[cont].valreal=dynamic_cast<TCCUnionEntreDepositos*>(BC[FSensorUED[cont].CCUED-1])->getGastoImpreso();
...
try
{
FNInputsdll=FNSensTubos+FNSensDepositos+FNSensTurbogrupo+FNSensTurbina+FNSensMotor+FNSensVenturis+FNSensCilindros+FNSensUED;
...
void TCalculoExtern::Calculo_Sensores_Tubos(TTubo **Pipe,double deltaT)
{
   try
   {
     double dist=0.,T=0.,temp=0.,v1=0.,v2=0.,den=0.,d=0.,p=0.,
            gto1=0.,gto2=0.,gamma,Rmezcla;
     int n1=0,j=0,n2=0;
     for(int i=0;i<=FNSensTubos-1;i++){
        ...
        if (n1==FSensorTubo[i].nodos-1){
            switch(FSensorTubo[i].tipo){
                case nmTempTubo: temp=Pipe[j]->GetAsonido(n1)*ARef;
                                 FSensorTubo[i].valreal=temp*temp/Pipe[j]->GetGamma(n1)/Pipe[j]->GetRMezcla(n1)-273.;
                                 ...
                                 break;
                case nmPrTubo: FSensorTubo[i].valreal=Pipe[j]->GetPresion(n1);
                               FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant + deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                               FSensorTubo[i].valant=FSensorTubo[i].valact;
                               FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                               break;
                case nmVelTubo: FSensorTubo[i].valreal=Pipe[j]->GetVelocidad(n1)*ARef;
                                FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                FSensorTubo[i].valant=FSensorTubo[i].valact;
                                FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                break;
                case nmGastoTubo: temp=Pipe[j]->GetAsonido(n1)*ARef;
      		                  T = temp*temp/Pipe[j]->GetGamma(n1)/Pipe[j]->GetRMezcla(n1)-273.;
                                  ...
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                  break;
                case nmN2Tubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboN2);
                               FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                               FSensorTubo[i].valant=FSensorTubo[i].valact;
                               FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                               break;
                case nmO2Tubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboO2);
                               FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                               FSensorTubo[i].valant=FSensorTubo[i].valact;
                               FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                               break;
                case nmCO2Tubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboCO2);
                                FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                FSensorTubo[i].valant=FSensorTubo[i].valact;
                                FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                break;
                case nmH2OTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboH2O);
                                FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                FSensorTubo[i].valant=FSensorTubo[i].valact;
                                FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                break;
                case nmHCTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboHC);
                               FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                               FSensorTubo[i].valant=FSensorTubo[i].valact;
                               FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                               break;
                case nmSootTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboSoot);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                 break;
                case nmNOxTubo:  FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboNOx);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                 break;
                case nmCOTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboCO);
                               FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                               FSensorTubo[i].valant=FSensorTubo[i].valact;
                               FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                               break;
                case nmAireFrTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboAireFresco);
                                   FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                   FSensorTubo[i].valant=FSensorTubo[i].valact;
                                   FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                   break;
                case nmGasQuemadoTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboGasQuemado);
                                       FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                       FSensorTubo[i].valant=FSensorTubo[i].valact;
                                       FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                       break;
                case nmEGRTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboEGR);
                                FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                FSensorTubo[i].valant=FSensorTubo[i].valact;
                                FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                break;
                case nmCombustibleTubo: FSensorTubo[i].valreal=Pipe[j]->GetFraccionMasica(n1,FIndiceTuboComb);
                                        FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                        FSensorTubo[i].valant=FSensorTubo[i].valact;
                                        FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                        break;
                ...
                d=dist-n1;
                switch(FSensorTubo[i].tipo){
                  case nmTempTubo: temp=xit_(Pipe[j]->GetAsonido(n1),Pipe[j]->GetAsonido(n2),1.0,d)*ARef;
                                  gamma=xit_(Pipe[j]->GetGamma(n1),Pipe[j]->GetGamma(n2),1.0,d);
                                  Rmezcla=xit_(Pipe[j]->GetRMezcla(n1),Pipe[j]->GetRMezcla(n2),1.0,d);
                                  FSensorTubo[i].valreal=temp*temp/gamma/Rmezcla-273.;
                                  FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant + deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                  FSensorTubo[i].valant=FSensorTubo[i].valact;
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                    break;
                  case nmPrTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetPresion(n1),Pipe[j]->GetPresion(n2),1.0,d);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant + deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                    break;
                  case nmVelTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetVelocidad(n1),Pipe[j]->GetVelocidad(n2),1.0,d)*ARef;
                                  FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant + deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                  FSensorTubo[i].valant=FSensorTubo[i].valact;
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                  break;
                  case nmGastoTubo:temp = xit_(Pipe[j]->GetAsonido(n1),Pipe[j]->GetAsonido(n2),1.0,d)*ARef;
                                   gamma=xit_(Pipe[j]->GetGamma(n1),Pipe[j]->GetGamma(n2),1.0,d);
                                   Rmezcla=xit_(Pipe[j]->GetRMezcla(n1),Pipe[j]->GetRMezcla(n2),1.0,d);
                                   T= temp*temp/gamma/Rmezcla-273.;
                                   ...
                                   FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                   break;
                  case nmN2Tubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboN2),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboN2),1.0,d);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                 break;
                  case nmO2Tubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboO2),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboO2),1.0,d);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                 break;
                  case nmCO2Tubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboCO2),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboCO2),1.0,d);
                                  FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                  FSensorTubo[i].valant=FSensorTubo[i].valact;
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                  break;
                  case nmH2OTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboH2O),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboH2O),1.0,d);
                                  FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                  FSensorTubo[i].valant=FSensorTubo[i].valact;
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                  break;
                  case nmHCTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboHC),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboHC),1.0,d);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                 break;
                  case nmSootTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboSoot),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboSoot),1.0,d);
                                   FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                   FSensorTubo[i].valant=FSensorTubo[i].valact;
                                   FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                   break;
                  case nmNOxTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboNOx),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboNOx),1.0,d);
                                  FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                  FSensorTubo[i].valant=FSensorTubo[i].valact;
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                  break;
                  case nmCOTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboCO),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboCO),1.0,d);
                                 FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                 FSensorTubo[i].valant=FSensorTubo[i].valact;
                                 FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                 break;
                  case nmAireFrTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboAireFresco),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboAireFresco),1.0,d);
                                     FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                     FSensorTubo[i].valant=FSensorTubo[i].valact;
                                     FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                     break;
                  case nmGasQuemadoTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboGasQuemado),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboGasQuemado),1.0,d);
                                         FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                         FSensorTubo[i].valant=FSensorTubo[i].valact;
                                         FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                         break;
                  case nmEGRTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboEGR),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboEGR),1.0,d);
                                  FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                  FSensorTubo[i].valant=FSensorTubo[i].valact;
                                  FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                  break;
                  case nmCombustibleTubo: FSensorTubo[i].valreal=xit_(Pipe[j]->GetFraccionMasica(n1,FIndiceTuboComb),Pipe[j]->GetFraccionMasica(n1,FIndiceTuboComb),1.0,d);
                                          FSensorTubo[i].valact=((2*FSensorTubo[i].ctetiempo-deltaT) * FSensorTubo[i].valant+deltaT * FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant)) / ( 2*FSensorTubo[i].ctetiempo + deltaT );
                                          FSensorTubo[i].valant=FSensorTubo[i].valact;
                                          FSensorTubo[i].valrealant=FSensorTubo[i].valreal;
                                          break;
   ...
   try
   {
    for(int i=0;i<=FNSensDepositos-1;i++){
        switch(FSensorDep[i].tipo){
                case nmPrDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->getPressure();
                              FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                              FSensorDep[i].valant=FSensorDep[i].valact;
                              FSensorDep[i].valrealant=FSensorDep[i].valreal;
                              break;
                case nmTempDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->getTemperature();
                                FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                                FSensorDep[i].valant=FSensorDep[i].valact;
                                FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                break;
				case nmN2Dep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboN2);
                              FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                              FSensorDep[i].valant=FSensorDep[i].valact;
                              FSensorDep[i].valrealant=FSensorDep[i].valreal;
                              break;
				case nmO2Dep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboO2);
                              FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                              FSensorDep[i].valant=FSensorDep[i].valact;
                              FSensorDep[i].valrealant=FSensorDep[i].valreal;
                              break;
				case nmCO2Dep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboCO2);
                              FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                              FSensorDep[i].valant=FSensorDep[i].valact;
                              FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                break;
				case nmH2ODep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboH2O);
                               FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                               FSensorDep[i].valant=FSensorDep[i].valact;
                               FSensorDep[i].valrealant=FSensorDep[i].valreal;
                               break;
				case nmHCDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboHC);
                              FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                              FSensorDep[i].valant=FSensorDep[i].valact;
                              FSensorDep[i].valrealant=FSensorDep[i].valreal;
                              break;
				case nmSootDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboSoot);
                                FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                                FSensorDep[i].valant=FSensorDep[i].valact;
                                FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                break;
				case nmNOxDep:  FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboNOx);
                                FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                                FSensorDep[i].valant=FSensorDep[i].valact;
                                FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                break;
				case nmCODep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboCO);
                              FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                              FSensorDep[i].valant=FSensorDep[i].valact;
                              FSensorDep[i].valrealant=FSensorDep[i].valreal;
                              break;
				case nmAireFrDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboAireFresco);
                                  FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                                  FSensorDep[i].valant=FSensorDep[i].valact;
                                  FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                  break;
				case nmGasQuemadoDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboGasQuemado);
                                      FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                                      FSensorDep[i].valant=FSensorDep[i].valact;
                                      FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                      break;
				case nmEGRDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboEGR);
                               FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                               FSensorDep[i].valant=FSensorDep[i].valact;
                               FSensorDep[i].valrealant=FSensorDep[i].valreal;
                               break;
				case nmCombustibleDep: FSensorDep[i].valreal=Plenum[FSensorDep[i].deposito-1]->GetFraccionMasicaEspecie(FIndiceTuboComb);
                                       FSensorDep[i].valact=((2*FSensorDep[i].ctetiempo-deltaT) * FSensorDep[i].valant + deltaT * FSensorDep[i].ganancia * (FSensorDep[i].valreal + FSensorDep[i].valrealant)) / ( 2*FSensorDep[i].ctetiempo + deltaT );
                                       FSensorDep[i].valant=FSensorDep[i].valact;
                                       FSensorDep[i].valrealant=FSensorDep[i].valreal;
                                       break;
   ...
   try
   {
    for(int i=0;i<=FNSensTurbogrupo-1;i++){
        switch(FSensorTG[i].tipo){
                case nmRTCTG:   FSensorTG[i].valreal=Axis[FSensorTG[i].turbogrupo-1]->getRegimen();
                                FSensorTG[i].valact=((2*FSensorTG[i].ctetiempo-deltaT) * FSensorTG[i].valant + deltaT * FSensorTG[i].ganancia * (FSensorTG[i].valreal + FSensorTG[i].valrealant)) / ( 2*FSensorTG[i].ctetiempo + deltaT );
   ...
   try
   {
    for(int i=0;i<=FNSensTurbina-1;i++){
        switch(FSensorTurbina[i].tipo){
                case nmRelaci1: FSensorTurbina[i].valreal=Turbine[FSensorTurbina[i].turbina-1]->GetRelacionCinematica(0);
                                FSensorTurbina[i].valact=((2*FSensorTurbina[i].ctetiempo-deltaT) * FSensorTurbina[i].valant + deltaT * FSensorTurbina[i].ganancia * (FSensorTurbina[i].valreal + FSensorTurbina[i].valrealant)) / ( 2*FSensorTurbina[i].ctetiempo + deltaT );
                                FSensorTurbina[i].valant=FSensorTurbina[i].valact;
                                FSensorTurbina[i].valrealant=FSensorTurbina[i].valreal;
                                break;
                case nmRelaci2: FSensorTurbina[i].valreal=Turbine[FSensorTurbina[i].turbina-1]->GetRelacionCinematica(1);
                                FSensorTurbina[i].valact=((2*FSensorTurbina[i].ctetiempo-deltaT) * FSensorTurbina[i].valant + deltaT * FSensorTurbina[i].ganancia * (FSensorTurbina[i].valreal + FSensorTurbina[i].valrealant)) / ( 2*FSensorTurbina[i].ctetiempo + deltaT );
                                FSensorTurbina[i].valant=FSensorTurbina[i].valact;
                                FSensorTurbina[i].valrealant=FSensorTurbina[i].valreal;
                                break;
   ...
   try
   {
    for(int i=0;i<=FNSensCilindros-1;i++){
        switch(FSensorCil[i].tipo){
                case nmTempCil: FSensorCil[i].valreal=Engine[0]->GetCilindro(FSensorCil[i].cilindro-1)->getTemperature();
                                FSensorCil[i].valact=((2*FSensorCil[i].ctetiempo-deltaT) * FSensorCil[i].valant + deltaT * FSensorCil[i].ganancia * (FSensorCil[i].valreal + FSensorCil[i].valrealant)) / ( 2*FSensorCil[i].ctetiempo + deltaT );
                                FSensorCil[i].valant=FSensorCil[i].valact;
                                FSensorCil[i].valrealant=FSensorCil[i].valreal;
                                break;
                case nmPrCil: FSensorCil[i].valreal=Engine[0]->GetCilindro(FSensorCil[i].cilindro-1)->getPressure();
                              FSensorCil[i].valact=((2*FSensorCil[i].ctetiempo-deltaT) * FSensorCil[i].valant + deltaT * FSensorCil[i].ganancia * (FSensorCil[i].valreal + FSensorCil[i].valrealant)) / ( 2*FSensorCil[i].ctetiempo + deltaT );
                              FSensorCil[i].valant=FSensorCil[i].valact;
                              FSensorCil[i].valrealant=FSensorCil[i].valreal;
                              break;
case nmMfCil : FSensorCil[i].valreal=Engine[0]->GetCilindro(FSensorCil[i].cilindro-1)->getMasaFuel();
                              FSensorCil[i].valact=((2*FSensorCil[i].ctetiempo-deltaT) * FSensorCil[i].valant + deltaT * FSensorCil[i].ganancia * (FSensorCil[i].valreal + FSensorCil[i].valrealant)) / ( 2*FSensorCil[i].ctetiempo + deltaT );
                              FSensorCil[i].valant=FSensorCil[i].valact;
                              FSensorCil[i].valrealant=FSensorCil[i].valreal;
                              break;
                case nmAFRCil: FSensorCil[i].valreal=Engine[0]->GetCilindro(FSensorCil[i].cilindro-1)->getAFR();
                               FSensorCil[i].valact=((2*FSensorCil[i].ctetiempo-deltaT) * FSensorCil[i].valant + deltaT * FSensorCil[i].ganancia * (FSensorCil[i].valreal + FSensorCil[i].valrealant)) / ( 2*FSensorCil[i].ctetiempo + deltaT );
                               FSensorCil[i].valant=FSensorCil[i].valact;
                               FSensorCil[i].valrealant=FSensorCil[i].valreal;
                               break;
   ...
   double prent,prgar,velent,vellat,gasent,gaslat;
    ...
    for(int i=0;i<=FNSensVenturis-1;i++){
        switch(FSensorVent[i].tipo){
                case nmPrEntVent: prent=Venturi[FSensorVent[i].venturi-1]->getPressure();
                                  ...
                                  break;
                case nmPrGarVent: prent=Venturi[FSensorVent[i].venturi-1]->getPressure();
prgar=prent/pow(1+Venturi[FSensorVent[i].venturi-1]->getgamma3()*pow(dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCLateral())->getValvula()->getCRecuperacion(),2.),Venturi[FSensorVent[i].venturi-1]->getGamma()/Venturi[FSensorVent[i].venturi-1]->getgamma1());
                                  FSensorVent[i].valreal=prgar;
                                  FSensorVent[i].valact=((2*FSensorVent[i].ctetiempo-deltaT) * FSensorVent[i].valant + deltaT * FSensorVent[i].ganancia * (FSensorVent[i].valreal + FSensorVent[i].valrealant)) / ( 2*FSensorVent[i].ctetiempo + deltaT );
                                  FSensorVent[i].valant=FSensorVent[i].valact;
                                  FSensorVent[i].valrealant=FSensorVent[i].valreal;
                                  break;
                case nmVelEntVent: if(dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCEntrada())->getSentidoFlujo()==nmEntrante) {
                                       signo=1;  /* Si el flujo es entrante signo de la velocidad positivo */
                                   }else if (dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCEntrada())->getSentidoFlujo()==nmSaliente){
                                       signo=-1; /* Si el flujo es saliente signo de la velocidad negativo */
                                   }
								   velent=dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCEntrada())->getVelocity()*signo*ARef;
                                   FSensorVent[i].valreal=velent;
                                   FSensorVent[i].valact=((2*FSensorVent[i].ctetiempo-deltaT) * FSensorVent[i].valant + deltaT * FSensorVent[i].ganancia * (FSensorVent[i].valreal + FSensorVent[i].valrealant)) / ( 2*FSensorVent[i].ctetiempo + deltaT );
                                   FSensorVent[i].valant=FSensorVent[i].valact;
                                   FSensorVent[i].valrealant=FSensorVent[i].valreal;
                                   break;
				case nmVelLatVent: vellat=dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCLateral())->getVelocity()*ARef;
                ...
                case nmGastoEntVent: gasent=-dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCEntrada())->getMassflow();  /*Massflow entrante positivo.Saliente negativo. Pedro */
                                     ...
                                     FSensorVent[i].valrealant=FSensorVent[i].valreal;
                                     break;
                case nmGastoLatVent: gaslat=dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi-1]->getCCLateral())->getMassflow();
...
void TCalculoExtern::Calculo_Sensores_Motor(double deltaT,TBloqueMotor **Engine,double AcumulatedTime)
{
   try
   {
    for(int i=0;i<=FNSensMotor-1;i++){
        switch(FSensorMotor[i].tipo){
case nmAngulo:  FSensorMotor[i].valreal=Engine[0]->getTheta();
                               FSensorMotor[i].valact=((2*FSensorMotor[i].ctetiempo-deltaT) * FSensorMotor[i].valant + deltaT * FSensorMotor[i].ganancia * (FSensorMotor[i].valreal + FSensorMotor[i].valrealant)) / ( 2*FSensorMotor[i].ctetiempo + deltaT );
                               FSensorMotor[i].valant=FSensorMotor[i].valact;
                               FSensorMotor[i].valrealant=FSensorMotor[i].valreal;
                               break;
case nmReg: FSensorMotor[i].valreal=Engine[0]->getRegimen();
                            FSensorMotor[i].valact=((2*FSensorMotor[i].ctetiempo-deltaT) * FSensorMotor[i].valant + deltaT * FSensorMotor[i].ganancia * (FSensorMotor[i].valreal + FSensorMotor[i].valrealant)) / ( 2*FSensorMotor[i].ctetiempo + deltaT );
                            FSensorMotor[i].valant=FSensorMotor[i].valact;
                            FSensorMotor[i].valrealant=FSensorMotor[i].valreal;
                            break;
   ...
   try
   {
    for(int i=0;i<=FNSensUED-1;i++){
        switch(FSensorUED[i].tipo){
                case nmGasto: FSensorUED[i].valreal=dynamic_cast<TCCUnionEntreDepositos*>(BC[FSensorUED[i].CCUED-1])->getGastoImpreso();
                              FSensorUED[i].valact=((2*FSensorUED[i].ctetiempo-deltaT) * FSensorUED[i].valant + deltaT * FSensorUED[i].ganancia * (FSensorUED[i].valreal + FSensorUED[i].valrealant)) / ( 2*FSensorUED[i].ctetiempo + deltaT );
...
for (int i=(int)strlen(origin)-1;i>=0;i--){
    if (origin[i]=='\\'){contpunto=i+1;break;}//ha encontrado el punto de la extension
}
