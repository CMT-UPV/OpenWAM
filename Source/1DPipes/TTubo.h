...
struct stFunDisCar {
	...
	stFunDisCar(double a0, double a1, double v0, double v1, double dt,
			double sg) {
		A_0 = a0;
		A_1 = a1;
		V_0 = v0;
...
//---------------------------------------------------------------------------
	...
	/*!Comment  */
	void LeeDatosGeneralesTuboXML(xml_node node_openwam, 			//!< Comment
			TBloqueMotor **Engine);

	/*!Comment  */
	void LeeDatosGeometricosTuboXML(xml_node node_openwam, //!< Comment
			double ene,              //!< Comment
