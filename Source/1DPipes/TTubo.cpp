		...
		if (FTipoCalcTempPared != nmTempConstante) {
			...
			if (FTipoTransCal != nmPipaEscape && FTipoTransCal != nmPipaAdmision) {
				...
				if (FTipRefrig == nmAgua)
					fscanf(fich, "%lf ", &FTRefrigerante); /* Esta en ºC */
			}
			else {
				FCoefExt = 1.;
				FEmisividad = 0.5;
...
if(FTipoCalcTempPared!=nmTempConstante){
 ...
 if(CC[FNodoIzq-1]->getTipoCC()==nmPipeToPlenumConnection){
   ...
   for(int k=0;k<Deposito[numDeposito-1]->getNUniones();k++){
	   if(Deposito[numDeposito-1]->GetCCDeposito(k)->getUnionDPF() && !PrimeraVez){
		  FHayDPFNodoIzq=true;
		  FTipoCanal[0]=Deposito[numDeposito-1]->GetCCDeposito(k)->GetTuboExtremo(0).TipoCanal;
		  FDPFEntradaTubo=Deposito[numDeposito-1]->GetCCDeposito(k)->GetTuboExtremo(0).DPF;
			 if(Deposito[numDeposito-1]->GetCCDeposito(k)->GetTuboExtremo(0).TipoExtremo==nmIzquierda){
				FNodoDPFEntrada=0;
			 }else{
 ...
 if(CC[FNodoDer-1]->getTipoCC()==nmPipeToPlenumConnection){
   ...
   for(int k=0;k<Deposito[numDeposito-1]->getNUniones();k++){
	   if(Deposito[numDeposito-1]->GetCCDeposito(k)->getUnionDPF() && !PrimeraVez){
          FHayDPFNodoDer=true;
		  FTipoCanal[1]=Deposito[numDeposito-1]->GetCCDeposito(k)->GetTuboExtremo(0).TipoCanal;
		  FDPFSalidaTubo=Deposito[numDeposito-1]->GetCCDeposito(k)->GetTuboExtremo(0).DPF;
			 if(Deposito[numDeposito-1]->GetCCDeposito(k)->GetTuboExtremo(0).TipoExtremo==nmIzquierda){
				FNodoDPFSalida=0;
			 }else{
...
void TTubo::IniciaVariablesFundamentalesTubo() {
	...
	try {
		...
		// Calculo de Gamma y R para la composición inicial.
		if (FCalculoEspecies == nmCalculoCompleto) {
...
void TTubo::ActualizaPropiedadesGas() {
	...
	try {
		...
		for (int i = 0; i < FNin; i++) {
			...
			// Calculo de Gamma y R a partir de la composición en cada nodo.
...
void TTubo::Transforma2(double& v, double& a, double& p, double **U, const double& Gamma,
	...
	try {
		...
		if (v > 1e200 || v < -1e200) {
			std::cout << "ERROR: Valor de velocidad no válido" << std::endl;
			throw Exception("Error Velociad");
		}
		if (p > 1e200 || p < 0) {
			std::cout << "ERROR: Valor de presion no válido en el tubo " << FNumeroTubo << " nodo " <<
				i << std::endl;
			throw Exception("Error presion");
		}
		if (a > 1e200 || a < 0) {
			std::cout << "ERROR: Valor de velocidad del sonido no válido" << std::endl;
			throw Exception("Error velocidad del sonido");
		}

		// Solución del Transporte de Especies Químicas.
...
void TTubo::Transforma3Area(double **Ufct, double **U, double Area, double Gamma, double Gamma1,
	...
	try {
		...
		for (int j = 3; j < FNumEcuaciones; j++) {
			Ufct[j][i] = U[j][i] * U[1][i] / U[0][i]; // Massflow de cada especie.
		}
#if usetry
	}
	catch(Exception & N) {
	...
	try {
		...
		if (!peta) {
			...
			for (int j = 0; j < FNumeroEspecies - 1 - FIntEGR; j++) {
				if (Ufctd[0][i] != 0.) {
					Y[j] = Ufctd[j + 3][i] / Ufctd[0][i]; // Fracción másica de cada especie.
				}
				else {
...
#endif
		int Nodos;
		double x1, x2, x3, x4, *hi12, *rho12, *Re12, *TPTubo12, *CoefTurbulencia12, *Gamma12,
		*Rmezcla12, *Gamma1_12;
		...
		CalculaFuente2(FU0, FV2, FDiametroTubo, FCoefTurbulencia, Fhi, Frho, FRe, FTPTubo[0],
		...
		CalculaFuente2(FU12, FV2, FDiametroD12, CoefTurbulencia12, hi12, rho12, Re12, TPTubo12,
		...
		// Liberación de memoria en vectores locales.
		delete[]hi12;
		delete[]rho12;
		delete[]Re12;
		delete[]TPTubo12;
	...
	try {
		...
		if (FNin > 3) {
			...
			if (FMod.Antidifusion == nmExplicit) {
				...
				std::cout << "ERROR: Metodo de Antidifusion mal definido para el FCT en el tubo n. " <<
					FNumeroTubo << std::endl;
				throw Exception("");
			}
...
void TTubo::LaxWendroffArea() {
	...
	try {
		...
		*Rmezcla12, *Gamma1_12;
		...
		CalculaFuente2Area(FU0, FV2, FArea, FCoefTurbulencia, Fhi, Frho, FRe, FTPTubo[0], FGamma,
		...
		CalculaFuente2Area(FU12, FV2, FArea12, CoefTurbulencia12, hi12, rho12, Re12, TPTubo12,
			Gamma12, Rmezcla12, Gamma1_12, Nodos);
...
void TTubo::CalculaFuente2(double **U, double **V2, double *diame, double *CoefTurbulencia,
	double *hi, double *rho, double *Re, double *TempParedTubo, double *Gamma, double *Rmezcla,
	...
	try {
		...
		for (int i = 0; i < Nodos; i++) {
			...
			if (p > 1e200 || p < 0) {
				std::cout << "ERROR: Valor de presion no válido en el tubo " << FNumeroTubo <<
					" nodo " << i << std::endl;
				throw Exception("Error presion");
			}
			if (a > 1e200 || a < 0) {
			...
			if (DoubEqZero(FCoefAjusTC)) {
				...
				TransmisionCalor(tgas, diame[i], q, CoefTurbulencia[i], hi[i], rho[i],
					TempParedTubo[i]);
				q = q * FCoefAjusTC;
			}
...
void TTubo::CalculaFuente2Area(double **U, double **V2, double *Area, double *CoefTurbulencia,
	double *hi, double *rho, double *Re, double *TempParedTubo, double *Gamma, double *Rmezcla,
	double *Gamma1, int Nodos) {
	...
	try {
		...
		for (int i = 0; i < Nodos; i++) {
			...
			if (DoubEqZero(FCoefAjusTC)) { /* q=0 */
				...
				tgas = a * a / (Gamma[i] * Rmezcla[i]);
				TransmisionCalor(tgas, diame, q, CoefTurbulencia[i], hi[i], rho[i],
					TempParedTubo[i]);
				q = q * FCoefAjusTC;
			}
...
void TTubo::TransmisionCalor(double tgas, double diametro, double& q, double CoefTurbulencia,
	...
	try {
		...
		if (hi != 0) {
			...
			case nmTuboEscape:
				q = 4. * hi * (Tw + 273. - tgas) / rho / diametro * CoefTurbulencia;
				break;
		...
		for (int i = 0; i < FNumResMedios; i++) {
			...
			for (int j = 0; j < NumVars; j++) {
				...
				case 7:
					...
					if (!HayMotor) {
						std::cout << "ERROR: No puedes pedir el NIT como resultado si no hay motor" <<
							std::endl;
						throw Exception(" ");
					}
					break;
				...
				default:
					std::cout <<
						"WARNING: El tipo de variable seleccionada para la salida de resultados medios no es válida" << std::endl;
				};
...
void TTubo::ReadInstantaneousResultsTubo(char *FileWAM, fpos_t &filepos, bool HayMotor) {
	...
	try {
		...
		fscanf(fich, "%d ", &FNumResInstant); // Puntos del tubo en los que se piden resultados instantáneos
		...
		for (int i = 0; i < FNumResInstant; i++) {
			...
			for (int j = 0; j < NumVars; j++) {
				...
				default:
					std::cout <<
						"WARNING: El tipo de variable seleccionada para la salida de resultados instantaneos no es válida" << std::endl;
				}
			}
		}
...
void TTubo::CalculaCoeficientePeliculaExterior(TBloqueMotor **Engine, double AmbientPressure,
	...
	try {
		...
		if (FTipoCalcTempPared != nmTempConstante && FCoefAjusTC != 0) {
			...
			if (FTipoTransCal == nmTuboAdmision || FTipoTransCal == nmTuboEscape) {
				for (int i = 0; i < FNin; i++) {
					...
					// Calculo de las características del refrigerante
					switch(FTipRefrig) {
					case nmAire:
					...
					case nmAgua:
						...
						break;
					default:
						std::cout << "WARNING: Tipo de refrigeración mal definida en el tubo: " <<
							FNumeroTubo << std::endl;
					}
				...
				// Longitud característica del conducto refrigerante.
				...
				for (int i = 0; i < FNin; i++) {
					...
					// Termino de conveccion de Seider-Tate con el término de Cipolla.
		...
		std::cout << "ERROR: TTubo::CalculaCoeficientePeliculaExterior en el tubo: " << FNumeroTubo <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
...
void TTubo::CalculaResistenciasdePared(TCondicionContorno **BC) {
	...
	try {
		...
		if (FTipoCalcTempPared != nmTempConstante && FCoefAjusTC != 0 && !FConcentrico){
			for (int i = 0; i < FNin; i++) {
				// Cálculo de las resistencias térmicas radiales.
				EsInterior = true;
				...
				for (int j = 0; j < FNumCapas; j++) {
					...
					if (FCapa[j].EsPrincipal == false) {
						...
						if (EsInterior) {
							// Cálculo de la resistencia térmica radial interior.
							if (FCapa[j].EsFluida) {
								// Capa de cámara de fluido.
							...
							// Cálculo de la resistencia térmica radial exterior.
							// Cálculo de la resistencia térmica radial interior.
							if (FCapa[j].EsFluida) {
								// Capa de cámara de fluido.
						...
						// Cálculo de la resistencia térmica radial exterior e interior de la capa principal.
				...
				// Cálculo de las resistencias térmicas axiales.
				FResistAxiAnt[i] = 0;
				FResistAxiPos[i] = 0;
				...
				if (i == 0) {
					...
					if(FHayDPFNodoIzq){
						if(FTipoCanal[0]==0){  // Junction to a DPF inlet channel
							FResistAxiAnt[i]=FDPFEntradaTubo->getAjustRAxAnt()/(Pi*FDPFEntradaTubo->getConductividadMetal()*(FDPFEntradaTubo->getDiametroEfect()+2*FDPFEntradaTubo->getEspesorAislante()+2*FDPFEntradaTubo->getEspesorAire()+FDPFEntradaTubo->getEspesorMetal())*FDPFEntradaTubo->getEspesorMetal());
						}else{  // Junction to a DPF outlet channel
							FResistAxiAnt[i]=FDPFEntradaTubo->getAjustRAxPos()/(Pi*FDPFEntradaTubo->getConductividadMetal()*(FDPFEntradaTubo->getDiametroEfect()+2*FDPFEntradaTubo->getEspesorAislante()+2*FDPFEntradaTubo->getEspesorAire()+FDPFEntradaTubo->getEspesorMetal())*FDPFEntradaTubo->getEspesorMetal());
						}
					}
				}
				#endif
					...
					if (BC[FNodoDer - 1]->getTipoCC() == nmPipesConnection) {
						...
						if(FHayDPFNodoDer){
							if(FTipoCanal[1]==0){  // Junction to a DPF inlet channel
								FResistAxiPos[i]=FDPFSalidaTubo->getAjustRAxAnt()/(Pi*FDPFSalidaTubo->getConductividadMetal()*(FDPFSalidaTubo->getDiametroEfect()+2*FDPFSalidaTubo->getEspesorAislante()+2*FDPFSalidaTubo->getEspesorAire()+FDPFSalidaTubo->getEspesorMetal())*FDPFSalidaTubo->getEspesorMetal());
							}else{    // Junction to a DPF outlet channel
								FResistAxiPos[i]=FDPFSalidaTubo->getAjustRAxPos()/(Pi*FDPFSalidaTubo->getConductividadMetal()*(FDPFSalidaTubo->getDiametroEfect()+2*FDPFSalidaTubo->getEspesorAislante()+2*FDPFSalidaTubo->getEspesorAire()+FDPFSalidaTubo->getEspesorMetal())*FDPFSalidaTubo->getEspesorMetal());
							}
						}
					}
					#endif
				...
				// Cálculo de las capacidades térmicas.
				EsInterior = true;
				Dint = FDiametroTubo[i];
				FCapInt[i] = 0.;
				FCapMed[i] = 0.;
				...
				for (int j = 0; j < FNumCapas; j++) {
					...
					if (FCapa[j].EsPrincipal == false) {
						if (EsInterior) {
							// Cálculo de la capacidad térmica interior.
							if (!FCapa[j].EsFluida) {
								// Capa de material.
							...
							// Cálculo de la capacidad térmica exterior.
							if (!FCapa[j].EsFluida) {
								// Capa de material.
								Cap = FCapa[j].Density * FCapa[j].CalorEspecifico * Pi / 4 *
									(pow2(Dext) - pow2(Dint)) * FXref;
						...
						// Cálculo de la capacidad térmica exterior, media e interior de la capa principal.
...
void TTubo::CalculaCoeficientePeliculaInterior(TCondicionContorno **BC) {
	...
	try {
		...
		if (FCoefAjusTC != 0) {
			for (int i = 0; i < FNin; i++) {
				...
				case nmPipaAdmision:
					...
					break;
				case nmTuboEscape:
					// Término de convección correlación M. Reyes
				...
				case nmPipaEscape:
					...
					break;
				default:
					std::cout << "WARNING: Transmisión de calor mal definida en el tubo: " <<
						FNumeroTubo << std::endl;
				}
			}
		}
		...
		std::cout << "ERROR: TTubo::CalculaCoeficientePeliculaInterior en el tubo: " << FNumeroTubo <<
			std::endl;
		std::cout << "Tipo de error: " << N.Message.c_str() << std::endl;
		throw Exception(N.Message.c_str());
	}
	...
	try {
		...
		if (FTipoCalcTempPared != nmTempConstante && FCoefAjusTC != 0) {
			...
			for (int i = 0; i < FNin; i++) {
				...
				// Establece la temperatura de la pared en el instante de cálculo anterior...
				...
				// Resistencias térmicas interior y exterior.
				if (DoubEqZero(Fhi[i])) {
					Ri = 100000000.;
				}
				else {
				...
				// Cálculo de las temperaturas de pared.
				...
				// Si el tipo de calculo es sin inercia termica ó lleva menos de "NumCiclosSinInerciaTermica" ciclos calculando...
				if (FTipoCalcTempPared == nmVariableSinInerciaTermica || Theta /
					...
					if (Fhi[i] != 0. && Theta > FAnguloTotalCiclo) {
						// Sumatorio de h*Tg*incrt intermedio(para el cálculo de la integral).
						FSUMTPTuboPro[0][1][i] += 1 / (Ri + FResistRadInt[i]) * Tg * DeltaTTPared;
						// Sumatorio de h*incrt intermedio(para el cálculo de la integral).
						FSUMTPTuboPro[1][1][i] += 1 / (Ri + FResistRadInt[i]) * DeltaTTPared;
						// Sumatorio de h*Tg*incrt interior(para el cálculo de la integral).
						FSUMTPTuboPro[0][0][i] += 1 / Ri * Tg * DeltaTTPared;
						// Sumatorio de h*incrt interior(para el cálculo de la integral).
			...
			// Si está al final del ciclo...
			if (FCicloTubo != Engine[0]->getCiclo() && FSUMTime > 0.) {
				// ...si (el tipo de calculo es sin inercia termica ó lleva menos de "NumCiclosSinInerciaTermica" ciclos calculando) y está en el segundo ciclo, calcula la temperatura de convergencia
				if( (FTipoCalcTempPared == nmVariableSinInerciaTermica || Theta /
					 FAnguloTotalCiclo <= Engine[0]->getNumCiclosSinInerciaTermica())
					 && Theta > FAnguloTotalCiclo + 1) {
					ErrorTp = 1.;
					EsPrimeraVez = true;
					while (ErrorTp >= 1) { // Itera hasta conseguir una diferencia entre las temperaturas de pared menor a 1ºC entre Steps.
						ErrorTp = 0.;
						for (int i = 0; i < FNin; i++) {
							// Establece la temperatura de la pared en el instante de cálculo anterior...
							...
							if (i == 0) {
								if (BC[FNodoIzq - 1]->getTipoCC() == nmPipesConnection) {
									if (dynamic_cast<TCCUnionEntreTubos*>(BC[FNodoIzq - 1])
										...
										() == FNumeroTubo) {
										extremo = 1;
										if (BC[FNodoIzq - 1]->GetTuboExtremo(extremo).TipoExtremo == nmIzquierda) {
										nodo = 0;
										}
										else {
		...
		if (DeltaTiempo < 1e-15 || DoubEqZero(FVelocidadDim[extremo])) {
			...
			if (signo * FVelocidad0[extremo] < 0.0) {
				...
				while (!valido) {
					...
					/* Fin del cálculo de la velocidad en el punto considerado */
...
void TTubo::Calculo_Entropia(double& entropia, double& velocidadp, int ind, double dist, int signo,
	...
	try {
		...
		velabs = 0., dacal = 0., dafric = 0., tptubop = 0., hip = 0., coffp = 0., rhop = 0.,
		Rep = 0., gamma5p = 0., gamma3p = 0., gamma1p = 0., gammap = 0., Rmezclap = 0.;
		int ind1 = 0;
		...
		if (dist == 0. || dist == 1.) {
			...
			if (FCoefAjusTC != 0 || FCoefAjusFric != 0) {
				if (signo == 1) {
					...
					coffp = Interpola(FCoefTurbulencia[ind], FCoefTurbulencia[ind1], 1., dist);
				}
				else if (signo == -1) {
					...
					coffp = Interpola(FCoefTurbulencia[ind], FCoefTurbulencia[ind1], 1., 1. - dist);
				}
			}
		...
		if (DoubEqZero(FCoefAjusTC)) {
			...
			TransmisionCalor(tgasp, diamep, q, coffp, hip, rhop, tptubop);
		...
		/* variacion de la entropia debida al término de friccion */
		/* --------------------------------------- */
...
void TTubo::Calculo_Caracteristica(double& caracteristica, double& velocidadp, double& asonidop,
	...
	try {
		...
		double tptubop = 0., hip = 0., coffp = 0., Rep = 0., rhop = 0., gammap, gamma1p, gamma5p,
		gamma3p, Rmezclap;
...
void TTubo::CalculaB() {
	...
	try {
		...
		double Remed, gamma, gamma1, Rmed, himed, rhomed, cturbmed, twallmed, Vmed, H1, H2, Hmed,
		Amed, rhoAmed;
		...
		for (int i = 0; i < FNin - 1; i++) {
			...
			if (FArea[i] != FArea[i + 1] || FCoefAjusFric != 0 || FCoefAjusTC != 0) {
				...
				if (FCoefAjusTC != 0) {
					...
					cturbmed = 0.5 * (FCoefTurbulencia[i] + FCoefTurbulencia[i + 1]);
					twallmed = 0.5 * (FTPTubo[0][i] + FTPTubo[0][i + 1]);

					tgas = Amed * Amed / gamma / Rmed;

					TransmisionCalor(tgas, FDiametroD12[i], q, cturbmed, himed, rhomed, twallmed);
...
void TTubo::CalculaBmen() {
	...
	try {
		...
		rhoAmed;
		double Remed, Rmed, himed, rhomed, cturbmed, twallmed;
		...
		for (int i = 1; i < FNin - 2; ++i) {
			for (int k = 0; k < 3; k++) {
				if (FTVD.DeltaW[k][i] == 0 || fabs(FTVD.LandaD[k][i]) == 1) {
					...
					FTVD.R[k][i] = ((double)FTVD.hLandaD[k][i - FTVD.hLandaD[k][i]] - FTVD.LandaD[k]
						[i - FTVD.hLandaD[k][i]]) * (FTVD.DeltaW[k][i - FTVD.hLandaD[k][i]]) /
						(((double)FTVD.hLandaD[k][i] - FTVD.LandaD[k][i]) * FTVD.DeltaW[k][i]);
				}
			}
			for (int k = 3; k < FNumEcuaciones; k++) {
				if (FTVD.W[k][i] == FTVD.W[k][i + 1] || fabs(FTVD.LandaD[k][i]) == 1) {
					FTVD.R[k][i] = 1.;
				}
				else {
					FTVD.R[k][i] = ((double)FTVD.hLandaD[k][i - FTVD.hLandaD[k][i]] - FTVD.LandaD[k]
